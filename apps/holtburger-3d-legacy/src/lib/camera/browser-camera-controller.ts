...
export interface BrowserCameraControllerOptions {
	readonly onChange: (state: FreeCameraState) => void;
	readonly initialState?: FreeCameraState;
	readonly config?: FreeCameraConfig;
	readonly requestAnimationFrame?: (callback: FrameRequestCallback) => number;
	readonly cancelAnimationFrame?: (handle: number) => void;
}

export class BrowserCameraController {
	readonly #onChange: (state: FreeCameraState) => void;
	readonly #config: FreeCameraConfig | undefined;
	readonly #requestAnimationFrame: (callback: FrameRequestCallback) => number;
	readonly #cancelAnimationFrame: (handle: number) => void;
	...
	constructor(options: BrowserCameraControllerOptions) {
		this.#onChange = options.onChange;
		this.#config = options.config;
		this.#state = options.initialState ?? createFreeCameraState(options.config);
	...
	createFrameStateCamera(): FrameState["camera"] {
		return createFreeCameraFrameStateCamera(this.#state);
	}

	createSnapshot(): FreeCameraState {
		return this.#state;
	}

	reset(): void {
		this.#stopMovement();
		this.#setState(createFreeCameraState(this.#config));
	}

	setState(state: FreeCameraState): void {
		this.#stopMovement();
		this.#setState(state);
	}

	setPosition(position: FreeCameraState["position"]): void {
		this.#setState({
			...this.#state,
			position,
		});
	...
	handlePointerMove(event: PointerEvent): boolean {
		...
		const speedMultiplier = getFreeCameraSpeedMultiplier(
			event.shiftKey,
			this.#config,
		);
		this.#setState(
			drag.mode === "rotate"
				? rotateFreeCamera(this.#state, delta, speedMultiplier, this.#config)
				: panFreeCamera(this.#state, delta, speedMultiplier, this.#config),
		);
	...
	handleWheel(event: WheelEvent): boolean {
		...
		this.#setState(
			moveFreeCameraLocalUpByWheel(
				this.#state,
				wheelDelta,
				getFreeCameraSpeedMultiplier(event.shiftKey, this.#config),
				this.#config,
			),
		);
	...
	#setState(nextState: FreeCameraState): void {
	...
	#applyMovement(frameAt: number): void {
		...
		if (yawDirection !== 0) {
			nextState = rotateFreeCameraAroundWorldUp(
				nextState,
				yawDirection,
				deltaSeconds,
				speedMultiplier,
