...
pub trait PhysicalFlyEventSink: Send + Sync {
    /// Publishes one authoritative motion path for the active camera generation.
    fn publish_motion(&self, path: PhysicalFlyMotionPath) -> anyhow::Result<()>;

    /// Publishes the terminal failure for a camera generation.
...
/// One physical fly runtime shared by narrow host commands and its tick task.
...
impl HostPhysicalFlyRuntime {
    ...
    pub fn schedule(self: &Arc<Self>, sink: Arc<dyn PhysicalFlyEventSink>, session: u64) -> bool {
        let mut state = self.state.lock().expect("camera runtime lock poisoned");
...
struct PhysicalFlyTickParticipant {
    sink: Arc<dyn PhysicalFlyEventSink>,
...
impl HostFixedTickParticipant for PhysicalFlyTickParticipant {
    ...
    fn fixed_tick_failed(&self, error: &anyhow::Error) {
        ...
        let _ = self.sink.publish_failure(PhysicalFlyFailure {
