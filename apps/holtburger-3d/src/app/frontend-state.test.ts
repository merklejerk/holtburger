...
function createRuntimeBatch(overrides: Partial<RuntimeBatchDto> = {}): RuntimeBatchDto {
  return {
    tick: 1,
    entities: [
      {
...
function createViewModelFeed(overrides: Partial<FrontendStateFeedDto> = {}): FrontendStateFeedDto {
  return {
...
describe('frontend state store', () => {
  ...
  it('merges runtime notifications inside the store boundary', () => {
    ...
    expect(get(store).host.boundarySnapshot?.runtimeBatch.residency.focusLandblockId).toBe(0x0102001b);
    expect(get(store).host.boundarySnapshot?.viewModelFeed.busyState).toBe('loading');
    expect(get(store).host.latestRuntimeNotification?.topic).toBe('runtime.batch');
  });
