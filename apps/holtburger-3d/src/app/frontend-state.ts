...
export function createInitialFrontendState(): FrontendAppState {
  return reconcileModeState({
    ...
    mode: createModeState('browser', 'location-entry', 'Browser mode is the default until lifecycle facts say otherwise.'),
  });
}
...
export function createFrontendStateStore() {
  const { subscribe, update } = writable<FrontendAppState>(createInitialFrontendState());

  return {
    subscribe,
    loadSnapshot(snapshot: HostBoundarySnapshot): void {
      update((state) => reconcileModeState(applyLoadedSnapshot(state, snapshot)));
    },
    applyRuntimeNotification(notification: RuntimeNotificationEnvelopeDto): void {
      update((state) => reconcileModeState(applyRuntimeNotification(state, notification)));
    },
    ...
    useRuntimeResidencyDestination(): void {
      update((state) => {
        ...
        return reconcileModeState({
          ...state,
          browserMode: selectRuntimeResidencyDestination(state.browserMode, residency),
        });
      });
    },
  };
...
function applyLoadedSnapshot(
  ...
  return {
    ...
    browserMode: seedBrowserDraftFromResidency(state.browserMode, snapshot.runtimeBatch.residency),
  };
}
...
function applyRuntimeNotification(
  ...
  const mergedSnapshot = mergeHostBoundarySnapshot(state.host.boundarySnapshot, notification);
  ...
  return {
    ...
    browserMode: seedBrowserDraftFromResidency(state.browserMode, mergedSnapshot.runtimeBatch.residency),
  };
}
...
function reconcileModeState(state: FrontendAppState): FrontendAppState {
  return {
    ...state,
    mode: deriveModeState(state.host.boundarySnapshot?.lifecycleState ?? null, state.browserMode),
  };
}
...
function createModeState(
  ...
  const activeModeSummary =
    availableModes.find((mode) => mode.id === activeMode)?.summary ?? 'Unknown mode summary.';
  const activeModeLabel =
    availableModes.find((mode) => mode.id === activeMode)?.label ?? 'Unknown Mode';
