 ...
 *
 * Mode-owned interaction, presentation defaults, and diagnostics live beside their composing
 * frontend. Values whose changes would alter decoded game data, renderer correctness, or API
	...
	rendering: {
		...
		nameplates: {
			...
			appearance: {
				...
				indicatorGapPixels: 5,
				indicatorSizePixels: 64,
				level: {
					fontSizePixels: 42,
					fontStyle: "normal",
					fontWeight: 400,
		 ...
		 *
		 * These values are the shared baseline. The Explorer grading panel emits its override block
		 * for `explorer/explorer-tuning.ts`; the client may select a different initial look.
		 */
		colorGrade: {
