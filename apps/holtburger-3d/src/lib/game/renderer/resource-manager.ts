...
import type { TerrainCompositionTable } from "../terrain/composition-table";
...
/** Opaque backend identity for one uploaded R32UI terrain surface texture. */
export type TerrainSurfaceResourceKey = `terrain-surface-resource:${number}`;

/** Opaque backend identity for one uploaded RGBA32UI terrain composition lookup texture. */
export type TerrainCompositionResourceKey =
	`terrain-composition-resource:${number}`;
...
export interface RendererResourceManager {
	...
	/** Upload one generated terrain surface field as a nearest-sampled integer texture. */
	createTerrainSurface(field: TerrainSurfaceField): TerrainSurfaceResourceKey;
	/** Upload one stable regional terrain composition table as an integer lookup texture. */
