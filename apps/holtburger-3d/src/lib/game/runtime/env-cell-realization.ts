...
function mergeGeometryDiagnostics(
	...
	return {
		...
		persistentCohortCount: sum((entry) => entry.persistentCohortCount),
		persistentDrawUnitCount: sum((entry) => entry.persistentDrawUnitCount),
		persistentInstanceCount: sum((entry) => entry.persistentInstanceCount),
		persistentStreamBytes: sum((entry) => entry.persistentStreamBytes),
		persistentStreamCount: sum((entry) => entry.persistentStreamCount),
