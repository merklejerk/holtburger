...
describe("OpenWorldTexturePageBuildTaskStream", () => {
	it("settles accepted page builds into renderer texture commits", async () => {
		...
		expect(fixture.registry.createBucketSnapshot(fixture.bucketKey).pages[0]).toMatchObject({
...
function createPageBuildInput(options: {
	...
	return {
		...
		entries: [
			{
				...
				entryId: `${options.bucketKey}:entry:1` as OpenWorldTexturePageBuildInput["entries"][number]["entryId"],
		...
		pageId: `${options.bucketKey}:page:1` as OpenWorldTexturePageBuildInput["pageId"],
