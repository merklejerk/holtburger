<script lang="ts">
	...
	const rendererDiagnosticsText = $derived.by(() => {
		...
		const bakedBypassSamples = summarizeSamples(
			debug.atlasBackedCompactionBypassSamples,
		);
		const bakedResourceFallbackSamples = summarizeSamples(
			debug.compactedGeometryResourceFallbackSamples,
		);
		const bakedSubmitFallbackSamples = summarizeSamples(
