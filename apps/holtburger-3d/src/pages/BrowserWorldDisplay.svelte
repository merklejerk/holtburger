<script lang="ts">
	...
	const rendererDiagnosticsText = $derived.by(() => {
		...
		const atlasCompactionBypassSamples = summarizeSamples(
			debug.atlasBackedCompactionBypassSamples,
		);
		const atlasResourceFallbackSamples = summarizeSamples(
			debug.compactedGeometryResourceFallbackSamples,
		);
		const atlasSubmitFallbackSamples = summarizeSamples(
			debug.atlasBackedCompactedSubmitFallbackSamples,
		);
		const stagedAtlasFallbackSamples = summarizeSamples(
			debug.stagedAtlasFallbackSamples,
		);
		const drawGroupTerm = "draw units";
		const performanceText = renderMetrics?.performance
			? `${renderMetrics.performance.fps.toFixed(1)} FPS, ${renderMetrics.performance.frameMs.toFixed(1)} ms/frame, ${renderMetrics.performance.renderMs.toFixed(1)} ms render`
			: "waiting for performance sample";
		return `Perf ${performanceText}. Diagnosis: ${diagnosis}. Draw pressure ${debug.renderCalls} visible draws from ${candidateBatchCount} candidate ${drawGroupTerm}; static candidates ${debug.staticBvhCandidateBatchCount}${staticCandidateRatio}; retained ${drawGroupTerm} terrain ${debug.terrainRenderBatchCount}, static ${debug.staticRenderBatchCount}, interiors ${debug.structuredInteriorRenderBatchCount}; retained tris ${debug.renderTriangles}. Material path ${debug.materialCount} materials, ${debug.textureResourceCount} textures, ${debug.indexedTextureResourceCount} indexed textures, ${debug.paletteResourceCount} palettes, direct/material types ${materialTypes}; atlas candidates ${debug.atlasEligibleMaterialCount} draws, ${debug.atlasCandidateEntryCount} entries, ${debug.atlasCandidateMaterialSlotCount} material slots; staged atlas draws ${debug.stagedAtlasDrawCount}, standalone direct draws ${debug.stagedAtlasStandaloneDirectDrawCount}, est. binds avoided ${debug.stagedAtlasEstimatedTextureBindAvoidedCount}, shared atlas textures ${debug.stagedAtlasSharedTextureAtlasTextureCount}${stagedAtlasFallbackSamples ? `, staged atlas fallbacks ${stagedAtlasFallbackSamples}` : ""}; atlas-backed compactable ${debug.atlasBackedCompactionCompactableDrawUnitCount}, base atlas textures ${debug.textureAtlasGenerationTextureCount}, detail atlas textures ${debug.detailTextureAtlasGenerationTextureCount}, compacted batches ${debug.compactedGeometryBatchCount}, compacted draws ${debug.compactedGeometryDrawUnitCount}, compacted slices ${debug.compactedGeometryDrawSliceCount}, batch origins ${debug.compactedGeometryBatchOriginCount}, transform table entries ${debug.compactedGeometryTransformTableEntryCount}, compacted bytes ${debug.compactedGeometryTotalByteLength}, shader draws ${debug.atlasBackedCompactedShaderDrawCallCount}, submitted batches ${debug.atlasBackedCompactedSubmittedBatchCount}, submitted slices ${debug.atlasBackedCompactedSubmittedDrawSliceCount}, represented slice draws ${debug.atlasBackedCompactedSubmittedSliceRepresentedDrawUnitCount}, replaced ${debug.atlasBackedCompactedReplacedDrawUnitCount}, retained ${debug.atlasBackedCompactedRetainedDrawUnitCount}, atlas draw-call math ${debug.atlasBackedCompactedOriginalDrawCallEstimateCount}-${debug.atlasBackedCompactedReplacedDrawUnitCount}+${debug.atlasBackedCompactedShaderDrawCallCount}=${debug.atlasBackedCompactedSubmittedDrawCallEstimateCount} saved ${debug.atlasBackedCompactedDrawCallSavingsCount}, atlas tris replaced/submitted ${debug.atlasBackedCompactedReplacedDrawUnitTriangleCount}/${debug.atlasBackedCompactedSubmittedTriangleCount}, conservative overdraw ${debug.atlasBackedCompactedConservativeOverdrawTriangleCount} tris (${debug.atlasBackedCompactedConservativeOverdrawRatio.toFixed(2)}), no-visible routes ${debug.atlasBackedCompactedSubmitNoVisibleRouteCount} (exterior ${debug.atlasBackedCompactedSubmitNoVisibleExteriorRouteCount}, interior ${debug.atlasBackedCompactedSubmitNoVisibleInteriorRouteCount}, other ${debug.atlasBackedCompactedSubmitNoVisibleOtherRouteCount}), bypasses ${debug.atlasBackedCompactionBypassReasonCount}${atlasCompactionBypassSamples ? ` (${atlasCompactionBypassSamples})` : ""}${atlasResourceFallbackSamples ? `, resource fallbacks ${atlasResourceFallbackSamples}` : ""}${atlasSubmitFallbackSamples ? `, submit fallbacks ${atlasSubmitFallbackSamples}` : ""}; fallbacks ${debug.fallbackReasonCount}${fallbackSamples ? ` (${fallbackSamples})` : ""}. Policy ${debug.renderGraphPolicy}, base ${debug.renderGraphBaseScene}, transition depth ${debug.transitionPortalMaxDepth}; canvas ${debug.canvasWidth}x${debug.canvasHeight} @${debug.pixelRatio.toFixed(2)}.`;
	});
