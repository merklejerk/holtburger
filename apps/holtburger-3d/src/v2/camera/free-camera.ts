...
export interface V2FreeCameraState {
...
export interface V2FreeCameraConfig {
...
export function createV2FreeCameraState(
	config = DEFAULT_V2_FREE_CAMERA_CONFIG,
): V2FreeCameraState {
	return {
...
export function rotateV2FreeCamera(
...
export function rotateV2FreeCameraAroundWorldUp(
	state: V2FreeCameraState,
	direction: -1 | 1,
	deltaSeconds: number,
	speedMultiplier = 1,
...
export function moveV2FreeCameraLocalUpByWheel(
	state: V2FreeCameraState,
	wheelDelta: number,
	speedMultiplier = 1,
	config = DEFAULT_V2_FREE_CAMERA_CONFIG,
): V2FreeCameraState {
	const { up } = getV2FreeCameraAxes(state);
...
export function panV2FreeCamera(
	state: V2FreeCameraState,
	deltaPixels: { readonly x: number; readonly y: number },
	speedMultiplier = 1,
	config = DEFAULT_V2_FREE_CAMERA_CONFIG,
): V2FreeCameraState {
	const { right, up } = getV2FreeCameraAxes(state);
	const panScale =
		state.focusDistance *
...
export function moveV2FreeCameraLocal(
	state: V2FreeCameraState,
	movement: {
	...
	const { forward, right, up } = getV2FreeCameraAxes(state);
...
export function getV2FreeCameraSpeedMultiplier(
...
export function getV2FreeCameraKeyboardMoveSpeedMultiplier(
...
export function createV2FreeCameraFrameStateCamera(
	state: V2FreeCameraState,
): FrameState["camera"] {
	return {
...
export function getV2FreeCameraAxes(
	state: Pick<V2FreeCameraState, "yawRadians" | "pitchRadians">,
): {
