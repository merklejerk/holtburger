...
describe("V2 landblock env-cell baker", () => {
	...
	it("omits portal aperture polygons from structured-interior draw units", () => {
		const input = createInputWithRenderableCellStructure({
			materialSources: [
				createStaticObjectMaterialSource({ materialId: 0x08000010 }),
				createStaticObjectMaterialSource({ materialId: 0x08000020 }),
			],
			renderSurfaces: [
		...
		} satisfies LandblockEnvCellStaticFacts;
		const scopedInput = replaceFirstEnvCell(input, portalEnvCell);

		const result = bakeLandblockEnvCells({
			...scopedInput,
			attachments: {
				envCellCellStructureGeometry: [
					createGeometryAttachment(portalEnvCell, {
		...
		expect(result.staticPortalInteriorRecords[0]).toMatchObject({
			envCells: [
				expect.objectContaining({
					portalApertures: [expect.objectContaining({ polygonId: 2 })],
					portals: [expect.objectContaining({ polygonId: 2 })],
				}),
			],
		});
	});
		...
		items: [
			{
				...
				payload: {
					...
					scope: {
						...scope,
						envCells: [envCell, ...scope.envCells.slice(1)],
					},
				},
			},
		],
