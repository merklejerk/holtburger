...
use crate::navigation::{
    DishonestNavigation, DishonestNavigationIntent as NavigationIntent,
    DishonestNavigationMode as NavigationMode,
    DishonestNavigationSyncInput as NavigationSyncInput, ResolvedNavigationTarget,
};
...
impl GameState {
        ...
        match action {
            ...
            AppAction::BeginInteraction { interaction } => {
                if interaction == Interaction::Salvaging {
                    ...
                    {
                        self.runtime.navigation.clear_navigation();
                        self.stop_active_navigation_drive(&mut result);
                    }
    ...
    pub fn handle_tick(&mut self, elapsed: f64) -> UpdateResult {
        ...
        self.sync_follow_target(now, &mut result);
        self.sync_sticky_melee_pursuit(&mut result);
        self.sync_navigation_drive(now, elapsed, &mut result);
    ...
    pub(crate) fn clear_active_interaction(&mut self, result: &mut UpdateResult) {
        if Self::is_frontend_navigation_interaction(self.view.active_interaction) {
            self.runtime.navigation.clear_navigation();
            self.stop_active_navigation_drive(result);
        }
    ...
    fn reconcile_navigation(
        ...
        if !matches!(
            ...
            log::trace!(
                "tui navigation: syncing {:?} target={:?} player_present={} auth_distance={:?} projected_distance={:?} run_rate={:.2}",
                intent,
        ...
        self.runtime.navigation.reconcile_navigation(intent, input);
    }
    ...
    fn start_approach_target(
        ...
        self.runtime.navigation.activate_approach(
            target,
            arrival_distance,
    ...
    fn start_follow_target(
        ...
        self.runtime.navigation.activate_follow(
            target,
            arrival_distance,
            self.navigation_sync_input(Instant::now(), Some(target)),
        );
    ...
    fn handle_forced_reposition(&mut self, result: &mut UpdateResult) {
        self.runtime.navigation.handle_forced_reposition();
        self.stop_active_navigation_drive(result);
        self.clear_finished_approach_interaction(result);
        if !matches!(
            self.runtime.navigation.navigation_mode(),
    ...
    fn handle_teleport_start(&mut self, result: &mut UpdateResult) {
        self.runtime.navigation.handle_teleport_start();
        self.stop_active_navigation_drive(result);
    ...
    fn sync_navigation_drive(&mut self, now: Instant, elapsed: f64, result: &mut UpdateResult) {
        let dt = Duration::from_secs_f64(elapsed.max(0.0));
        let drive_intent = self.active_navigation_target_guid().and_then(|guid| {
            self.runtime
                .navigation
                .active_drive_intent(self.navigation_sync_input(now, Some(guid)), dt)
        });
        ...
        match drive_intent {
            Some(intent) => {
                result
                    .commands
                    .push(ClientCommand::DriveSelf(PlayerDriveIntent::Autonomous(intent)));
                self.runtime.navigation_drive_active = true;
            }
            None => self.stop_active_navigation_drive(result),
        }
    }

    fn stop_active_navigation_drive(&mut self, result: &mut UpdateResult) {
        if !self.runtime.navigation_drive_active {
            return;
        }

        result
            .commands
            .push(ClientCommand::DriveSelf(PlayerDriveIntent::Stop));
        self.runtime.navigation_drive_active = false;
    }
...
struct GameRuntimeState {
    ...
    navigation: DishonestNavigation,
    navigation_drive_active: bool,
...
mod tests {
    ...
    fn has_active_dishonest_drive(state: &GameState) -> bool {
        let Some(target_guid) = state.active_navigation_target_guid() else {
            return false;
        };
        ...
        state
            ...
            .navigation
            .active_drive_intent(
                state.navigation_sync_input(Instant::now(), Some(target_guid)),
    ...
    fn player_movement_event_does_not_immediately_redrive_approach() {
        ...
        seed_active_approach(
            ...
            NavigationSyncInput {
                ...
                run_rate: DEFAULT_APPROACH_RUN_RATE,
            },
        );
    ...
    fn handle_tick_emits_autonomous_drive_for_active_approach() {
        ...
        assert!(state.runtime.navigation_drive_active);
    }
    ...
    fn handle_tick_emits_stop_when_navigation_drive_goes_idle() {
        ...
        state.runtime.navigation_drive_active = true;
    ...
    fn forced_reposition_cancels_frontend_owned_approach_controller() {
        ...
        assert!(has_active_dishonest_drive(&state));
        assert!(has_active_approach(&state));
        assert_eq!(
            state.view.active_interaction,
    ...
    fn remote_forced_reposition_updates_target_position_and_restarts_follow_when_out_of_range() {
        ...
        let _result = state.handle_view_event(ClientViewEvent::ForcedReposition {
            guid: target_guid,
            pos: WorldPosition {
                landblock_id: Guid(0x01000000),
    ...
    fn follow_keeps_interaction_after_arrival_and_restarts_when_target_moves() {
        ...
        let _in_range = state.handle_view_event(ClientViewEvent::EntityMoved {
            guid: target_guid,
            pos: WorldPosition {
                landblock_id: Guid(0x01000000),
                coords: holtburger_common::Vector3::new(FOLLOW_DISTANCE * 0.5, 0.0, 0.0),
        ...
        let _slipped = state.handle_view_event(ClientViewEvent::EntityMoved {
            guid: target_guid,
            pos: WorldPosition {
                landblock_id: Guid(0x01000000),
                coords: holtburger_common::Vector3::new(6.0, 0.0, 0.0),
    ...
    fn cancel_interaction_stops_active_follow_and_clears_mode() {
        ...
        let _result = state.handle_action(AppAction::CancelInteraction).unwrap();
    ...
    fn teleport_start_cancels_frontend_owned_approach_controller() {
        ...
        let _result = state.handle_view_event(ClientViewEvent::TeleportStarted { sequence: 7 });
    ...
    fn target_moving_beyond_tracking_distance_cancels_active_approach() {
        ...
        let _result = state.handle_view_event(ClientViewEvent::EntityMoved {
            guid: target_guid,
            pos: WorldPosition {
                landblock_id: Guid(0x01000000),
                coords: holtburger_common::Vector3::new(385.0, 0.0, 0.0),
    ...
    fn sticky_melee_keeps_repeat_latch_after_temporarily_returning_to_range() {
        ...
        let _in_range = state.handle_tick(0.016);
        ...
        let _slipped_again = state.handle_tick(0.016);
