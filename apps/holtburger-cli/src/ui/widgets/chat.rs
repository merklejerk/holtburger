...
pub fn render_chat_pane(f: &mut Frame, game: &mut GameState, app: &mut AppState, area: Rect) {
    ...
    let effective_scroll = game.scroll_offset;
    ...
    let chat_style = if game.focused_pane == FocusedPane::Chat {
        Style::default().fg(Color::Yellow)
    } else {
        Style::default()
    };

    let chat_title = if game.focused_pane == FocusedPane::Chat {
        ">> World Chat <<"
    } else {
        " World Chat "
    };
    ...
    let chat_list = List::new(messages).block(
        Block::default()
            ...
            .title_style(if game.focused_pane == FocusedPane::Chat {
                Style::default()
                    .fg(Color::Yellow)
...
pub fn render_context_pane(f: &mut Frame, game: &mut GameState, app: &mut AppState, area: Rect) {
    ...
    let mut ctx_items: Vec<ListItem<'static>> = game.context_buffer[ctx_start..ctx_end]
        .iter()
    ...
    let ctx_style = if game.focused_pane == FocusedPane::Context {
        Style::default().fg(Color::Yellow)
    } else {
        Style::default()
    };

    let base_title = match game.context_view {
    ...
    let ctx_title = if game.focused_pane == FocusedPane::Context {
        format!(">> {} <<", base_title)
    } else {
        format!(" {} ", base_title)
    };
    ...
    let ctx_list = List::new(ctx_items).block(
        Block::default()
            ...
            .title_style(if game.focused_pane == FocusedPane::Context {
                Style::default()
                    .fg(Color::Yellow)
                    .add_modifier(Modifier::BOLD)
            } else {
