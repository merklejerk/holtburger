...
pub fn render_character_tab(f: &mut Frame, game: &mut GameState, app: &mut AppState, area: Rect) {
    let mut bottom_area = area;

    if let Some(info) = &game.level_info {
    ...
    let items = get_stats_list_items(game, app);
    let total = items.len();
    ...
    f.render_stateful_widget(dashboard_list, bottom_area, &mut game.dashboard_list_state);
...
fn get_stats_list_items(game: &GameState, _app: &AppState) -> Vec<ListItem<'static>> {
    ...
    for (i, line) in items.iter().enumerate() {
        let highlight = i == game.selected_dashboard_index
            && matches!(
                line,
        ...
        match line {
            ...
            CharTabLine::Miscellaneous(enchant) => {
                ...
                let name = get_enchantment_name(enchant, &game.spell_names);
                let time_str = format_duration(enchant.start_time, enchant.duration);
                list_items.push(
                    ListItem::new(Line::from(vec![
                        Span::raw("  "),
...
pub fn get_char_tab_lines(game: &GameState) -> Vec<CharTabLine> {
    ...
    for enchant in &game.player_enchantments {
    ...
    let sort_by_name = |list: &mut Vec<Enchantment>| {
        list.sort_by(|a, b| {
            let na = get_enchantment_name(a, &game.spell_names);
            let nb = get_enchantment_name(b, &game.spell_names);
    ...
    if game.vitae < 0.999 || !vitae_enchants.is_empty() {
        let penalty_pct = (1.0 - game.vitae) * 100.0;
    ...
    let mut vitals: Vec<_> = game.vitals.values().collect();
    ...
    let mut attrs: Vec<_> = game.attributes.values().collect();
    ...
    if game.player_guid.is_some() {
        ...
        let mut resists = vec![
            (PropertyFloat::ResistSlash, game.resistances.slash),
            (PropertyFloat::ResistPierce, game.resistances.pierce),
            (PropertyFloat::ResistBludgeon, game.resistances.bludgeon),
            (PropertyFloat::ResistFire, game.resistances.fire),
            (PropertyFloat::ResistCold, game.resistances.cold),
            (PropertyFloat::ResistAcid, game.resistances.acid),
            (PropertyFloat::ResistElectric, game.resistances.electric),
            (PropertyFloat::ResistNether, game.resistances.nether),
        ];
