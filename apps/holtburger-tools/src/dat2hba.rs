...
use holtburger_dat::file_type::{Animation, EnvCell, GfxObj, SetupModel};
use holtburger_dat::{
    ...
    if state.should_prune_records {
        match file_type {
            ...
            DatFileType::Animation => {
                let mut cursor = std::io::Cursor::new(&data);
                if let Ok(mut animation) = Animation::read(&mut cursor) {
                    animation.prune_to_simulation_facts();
                    let mut pruned_data = Vec::new();
                    let mut out_cursor = std::io::Cursor::new(&mut pruned_data);
                    if animation.write(&mut out_cursor).is_ok() {
                        data = pruned_data;
                        is_pruned = true;
                        state.pruned_count.fetch_add(1, Ordering::SeqCst);
                    }
...
mod tests {
    ...
    #[test]
    fn micro_profile_keeps_required_table_ids_and_the_complete_motion_representation() {
    ...
    /// must still decode, because a profile that ships unreadable bytes is worse than one that
    /// ships none.
    #[test]
    fn animation_pruning_keeps_simulation_facts_and_drops_presentation() {
        ...
        let sound = AnimationHook {
            hook_type: 1,
            direction: 0,
            payload: AnimationHookPayload::Raw(vec![1, 2, 3, 4]),
        };
        let part_pose = Frame {
            origin: Vector3::new(9.0, 9.0, 9.0),
        ...
        let mut animation = Animation {
            ...
            num_parts: 2,
            num_frames: 2,
            pos_frames: vec![root.clone(), root.clone()],
            part_frames: vec![
                AnimationFrame {
                    frames: vec![part_pose.clone(), part_pose.clone()],
                    hooks: vec![attack.clone(), sound.clone()],
                },
                AnimationFrame {
                    frames: vec![part_pose.clone(), part_pose.clone()],
                    hooks: vec![sound.clone()],
                },
            ],
        };
        animation.prune_to_simulation_facts();
        ...
        animation
            ...
            .expect("pruned animation should re-emit");
        let decoded =
            Animation::read(&mut Cursor::new(bytes)).expect("pruned animation should decode");

        assert_eq!(decoded.num_parts, 0);
        assert_eq!(decoded.num_frames, 2);
        assert_eq!(decoded.pos_frames, vec![root.clone(), root]);
        assert!(
            decoded
                .part_frames
                .iter()
        ...
        assert_eq!(decoded.part_frames[0].hooks.len(), 1);
        assert_eq!(decoded.part_frames[0].hooks[0].payload, attack.payload);
        assert!(decoded.part_frames[1].hooks.is_empty());
    }
