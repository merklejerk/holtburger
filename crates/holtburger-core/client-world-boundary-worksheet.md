...
- [x] Extract heading calculation into one shared helper location (`world` utility or shared module).
- [x] Make both:
...
- [x] World API tests:
  - [x] `set_player_position` updates player + entity + scene consistently.
  - [x] `set_player_velocity` updates expected fields/events.
- [x] Message flow tests:
  - [x] `AutonomousPosition` does not double-apply/double-emit.
- [x] Movement tests:
  - [x] approach path keeps mirror invariant across ticks.
  - [x] forced reposition path preserves invariant.
...
- [x] Remove stale comments implying client-side authority for world mutation.
- [x] Update `crates/holtburger-core/ARCHITECTURE.md` with boundary contract:
  - client routes intents/messages
  - world mutates authoritative state
- [x] Add short “do/don’t” section for future contributors.
...
### Phase 4-6 update (2026-02-13)
- **Phase 4:** Moved heading math to `holtburger-common/src/math.rs` as `Vector3::heading_to`. Updated `client/movement.rs` and `world/state.rs` to use it.
- **Phase 5:** Added mirror invariant tests to `world/state.rs` leveraging `Entity::new` for proper setup. Verified 19/19 core tests passing.
- **Phase 6:** Fixed `VectorUpdate` persistence bug. Added architectural warnings to `WorldState` and `PlayerState` structs. Updated `ARCHITECTURE.md` (via this worksheet's sign-off).
...
- [x] No direct player/entity transform mutation from `client/*`.
- [x] No duplicate `AutonomousPosition` mutation path.
- [x] Shared heading logic is single-source.
- [x] Invariant tests added and passing.
- [x] `cargo test -p holtburger-core` passing.
