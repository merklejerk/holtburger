...
pub(super) fn tick(
    ...
    if let Some(collision) = collision {
        ...
        events.extend(tick_physical_entities(
            now,
            dt,
            world,
            movement,
    ...
    let prepared = world.scene.prepare_dynamic_entity_collection(
        ...
        |body| {
            if matches!(body.id, SpatialBodyId::LocalPlayer(_)) {
                local_player_actuation(body, dt, manual_offset, local_drive, local_object_scale)
            } else {
...
pub(super) async fn handle_server_controlled_movement(
    ...
    let Some(solved) = build_server_controlled_result(data, world) else {
        return Ok(Vec::new());
    };
    ...
    match &data.data {
        MovementTypeData::MoveToObject(mto) => {
            ...
            if (current_pos.landblock_id >> 16) == (mto.origin.cell_id >> 16) {
                next_pos.coords =
                    calculate_arrival_position(&current_pos, &mto.origin.position, arrival_dist);

                if mto.params.desired_heading.abs() <= 1e-6 {
                    next_pos.rotation =
                        Quaternion::from_heading(next_pos.coords.heading_to(&mto.origin.position));
                } else {
        ...
        MovementTypeData::MoveToPosition(mtp) => {
            ...
            if mtp.params.desired_heading.abs() > 1e-6 {
                ...
                next_pos.rotation =
                    Quaternion::from_heading(current_pos.coords.heading_to(&mtp.origin.position));
            }
        }
        ...
        MovementTypeData::TurnToObject(tto) => {
            ...
            {
                next_pos.rotation =
                    Quaternion::from_heading(next_pos.coords.heading_to(&target.position.coords));
            }
        }
        _ => {}
    }
...
mod tests {
    ...
    fn move_to_position_without_desired_heading_uses_current_pose_for_facing() {
        ...
        let solved = build_server_controlled_result(
            &MovementEventData {
                guid: player_guid,
