...
fn test_hba_parity_with_retail_portal() {
    ...
    for id in kept_ids {
        ...
        let hba_data = hba_reader.get_file(id).expect("Failed to get file from HBA");
    ...
    println!("SUCCESS: Verified parity for {}/{} essential files.", verified_count, dat_db.files.len());
}
