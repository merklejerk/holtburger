...
mod tests {
    ...
    fn test_create_object_minimal() {
        // The fixture contains only the body, missing the opcode.
    ...
    fn test_update_property_int_pack_private() {
        ...
        let mut packed = Vec::new();
        msg.pack(&mut packed);
        assert_eq!(hex::encode(&packed), "0c1900000032000000");
    }
    ...
    fn test_update_property_int_pack_public() {
        ...
        let mut packed = Vec::new();
        msg.pack(&mut packed);
        assert_eq!(hex::encode(&packed), "42785634121900000032000000");
    }
    ...
    #[test]
    fn test_private_update_property_int_buddy() {
    ...
    #[test]
    fn test_public_update_property_int_fixture() {
        let expected = PublicUpdatePropertyIntData {
            sequence: 0x42,
            guid: Guid(0x12345678),
            property: 25,
            value: 50,
        };
        // Hex: 42785634121900000032000000
        let data = hex::decode("42785634121900000032000000").unwrap();
        assert_pack_unpack_parity(&data, &expected);
    }
    ...
    fn test_update_property_float_pack() {
        ...
        let mut packed = Vec::new();
        msg.pack(&mut packed);
        assert_eq!(hex::encode(&packed), "0c190000000000000000005940");
    }
    ...
    fn test_update_property_float_fixture() {
        ...
        // Hex: 0C190000000000000000005940
        let data = hex::decode("0C190000000000000000005940").unwrap();
        assert_pack_unpack_parity(&data, &expected);
    }
    ...
    fn test_update_property_string_pack() {
        ...
        let mut packed = Vec::new();
        msg.pack(&mut packed);
        assert_eq!(hex::encode(&packed), "01010000000000000500416c69636500");
    }
    ...
    fn test_update_property_string_fixture() {
        ...
        // Hex: 01010000000000000500416C69636500
        let data = hex::decode("01010000000000000500416C69636500").unwrap();
        assert_pack_unpack_parity(&data, &expected);
    }
    ...
    #[test]
    fn test_force_obj_desc_send_fixture() {
        let expected = ForceObjectDescSendData {
            guid: Guid(0x50000001),
        };
        let data = &test_fixtures::FORCE_OBJ_DESC_SEND[4..];
        assert_pack_unpack_parity(data, &expected);
    }
    ...
    #[test]
    fn test_obj_desc_event_fixture() {
        let mut offset = 0;
        ...
        if let GameMessage::ObjDescEvent(ev) = msg {
            ...
            assert_eq!(ev.visual_desc_sequence, 5678);
        } else {
    ...
    fn test_obj_desc_event_drop_unpack() {
        let mut offset = 0;
        let data = test_fixtures::OBJ_DESC_EVENT_DROP;
    ...
    #[test]
    fn test_pickup_event_parity() {
        let expected = GameMessage::PickupEvent(Box::new(PickupEventData {
            guid: Guid(0x50000001),
            success: true,
        }));
        let mut packed = Vec::new();
        expected.pack(&mut packed);
        assert_pack_unpack_parity(&packed, &expected);
    }
}
