...
pub(crate) fn handle_message(
	...
	match message {
		...
		GameMessage::PrivateUpdateAttribute(data) => {
			...
			state
				.player
				.update_attribute(*attribute, *ranks, *start, *xp, state.xp_table.as_ref(), events);
			true
		}
		...
		GameMessage::PrivateUpdateVital(data) => {
			...
			state
				.player
				.update_vital(*vital, *ranks, *start, *current, *xp, state.xp_table.as_ref(), events);
			true
		}
...
pub(crate) fn handle_event(
	...
	match &event.event {
		...
		GameEvent::MagicUpdateEnchantment(data) => state
			.player
		...
		GameEvent::MagicRemoveEnchantment(data) => state
			.player
			.remove_enchantment(data.target, data.spell_id, data.layer, events),
		GameEvent::MagicDispelEnchantment(data) => state
			.player
			.remove_enchantment(data.target, data.spell_id, data.layer, events),
		...
		GameEvent::UpdateHealth(data) => state
			.player
