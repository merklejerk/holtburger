...
impl PlayingMotionClip {
    fn of(node: &SequenceNode) -> Self {
        Self {
            animation_id: node.animation().id,
            framerate: node.framerate(),
            low_frame: node.low_frame(),
            high_frame: node.high_frame(),
        }
    }
}
...
impl MotionRuntimeRegistry {
    ...
    pub fn drive(
        ...
        let runtime = self.bodies.entry(guid).or_insert_with(|| {
            let mut runtime = BodyMotionRuntime {
        ...
        apply_order(table, &mut runtime.state, &mut runtime.sequence, order);
        runtime.tick = runtime.sequence.advance(quantum);
        &runtime.tick
    }
}
...
fn apply_order(
    ...
    match order.forward {
        ...
        None => {
            ...
            // `stop_motion` does for whatever substate is currently running.
