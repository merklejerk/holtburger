...
2. **EntryCount (`uint32`)**: The number of actual file entries stored in _this_ node.
...
Raw textures have a specialized header _before_ the image data:
...
#### Environment and CellStruct Records

Environment records contain reusable CellStructs. Each CellStruct stores a vertex array, drawing
polygons, portal polygon IDs, a Cell BSP, physics polygons and BSP, and an optional drawing BSP.
An EnvCell in the cell DAT selects one Environment/CellStruct pair, supplies its own surface IDs
and landblock-space placement, and therefore owns material selection and transform even when
another EnvCell shares the same CellStruct geometry.

Portal polygon IDs name material-free planar apertures in addition to any independently valid
surface-bearing polygon side. Runtime portal topology comes from the EnvCell records, not from the
CellStruct portal list alone.
...
#### Indoor EnvCell Records

An EnvCell record contains its ID/flags, authored cell ID, surface selectors,
Environment/CellStruct selectors, placement, directed portals, potentially-visible cell IDs, and
optional static residents/restriction object.

Each portal stores `flags`, `polygon_id`, `other_cell_id`, and `other_portal_id`. The currently
proven flag meanings used by the 3D integration are:

|   Flag | Meaning                                                                        |
| -----: | ------------------------------------------------------------------------------ |
| `0x01` | `ExactMatch` authored portal fact                                              |
| `0x02` | Directed crossing accepts the portal plane's positive side; otherwise negative |
| `0x04` | Target is outside rather than another EnvCell                                  |

Internal reciprocal identity is valid only when the target portal points back to the source.
Outside endpoints may be claimed by a landblock building portal. The `visible_cells` list is
retained as potentially-visible/preload provenance; it is not treated as a complete per-view
visibility answer.

See [portal_rendering.md](portal_rendering.md) for the derived topology, containment, and rendering
contracts. Those derived contracts are not additional DAT fields.
