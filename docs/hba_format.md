...
| `3` | `Micro` | Minimal portal archive mode for the current TUI/runtime path. Contains the explicitly selected required table IDs plus all motion tables and animation records, and may contain pruned records if applicable. |
...
For the current terminal client, the smallest supported bundle is a portal-only micro archive containing the skill, spell, and XP tables plus all motion tables and animations required for motion-table-derived self movement.
