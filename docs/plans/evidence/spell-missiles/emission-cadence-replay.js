(async()=>{
const url=performance.getEntriesByType('resource').map(e=>e.name).find(n=>n.includes('/systems/particle-system.ts'));
...
for(const [name,info] of [['lightning',lightning],['force',force]])for(const hz of [30,60,144])for(const multiplier of [1,5,10]){
 ...
 let firstCap=null,lastCount=0,lastBirth=null,maxGap=0;const births=[];
 for(let step=1;step<=hz*3;step++){t=step/hz;origin=[15*t,0,0];ps.advance(t);const d=ps.getDiagnostics();if(d.emittedTotal>lastCount){births.push({t,x:origin[0]});if(lastBirth!==null)maxGap=Math.max(maxGap,t-lastBirth);lastBirth=t;lastCount=d.emittedTotal;}if(firstCap===null&&d.particleCount===20)firstCap=t;}
