...
| Landblock env-cell bundle     | Landblock-owned env-cell/topology facts resolved through the V2 `landblock-env-cells` source domain: classification, env-cell graph membership, portals/links, accepted/visible cell sets, local cell structures, static seeds, residency spatial records, and prepared BVHs. The old host topology route may exist for V1/debug transport, but V2 should not use it as a runtime source domain.                                                                                                              | landblock topology route, topology sidecar, indoor env-cell discovery, dungeon special case |
