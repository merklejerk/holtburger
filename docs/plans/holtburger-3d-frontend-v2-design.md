...
- Alpha-test/cutout surfaces are depth-writing discard materials and can participate in opaque-pass static batching when the draw-unit material table carries the alpha-test threshold. True alpha-blended/translucent/inverse-alpha/additive surfaces are different: they need typed blend/depth state and sortable transparent submissions, so they must not be folded into the same order-independent compacted batches as opaque/cutout geometry.
...
- Wrap intent is a material/texture-use sampling fact. For atlas-backed sampling paths that support shader-local virtual wrap, clamp/repeat should be applied against the local atlas rect in shader and should not split otherwise compatible physical atlas pages.
- Atlas capacity and material-table capacity create draw-unit slicing even when shader and material family match.

Conclusion: V2 should separate "what an AC material means" from "which static rendering bucket can draw this surface". A material family should remain a small shader/binding-layout compatibility class. A static draw unit should be the fuller compatibility product: family plus domain, pass/order constraints, sampler/device state, logical texture binding layout, placement revision assumptions, material-table partition, compacted geometry, sort/visibility partition facts, and draw slices. Order-independent opaque/cutout units may compact broadly; true blended units should be partitioned at object/part granularity for renderer distance sorting rather than at triangle granularity.
...
| Static domain                 | Static rendering/residency domain such as outdoor terrain, landblock topology, env-cell static, outdoor buildings/detail, portal mask, or dungeon env cells. Domains express which landblock-owned source family is being resolved, not whether the landblock is "outdoor" or "dungeon". `outdoor-detail` primarily covers generated outdoor scenery such as trees/foliage; explicit non-building outdoor objects may join only when evidence shows they belong there.                                                               | landblock render product, bundle kind                             |
...
sequenceDiagram
  ...
  Baker-->>Static: enrichment draw units + placement requirements + static records
...
Baking is static-only. It combines material classification and VAO-compaction concerns into one compatibility problem: if two surfaces cannot share shader family, pass/order class, sampler/device state, logical texture binding layout, capacity limits, domain, ownership scope, sort policy, or visibility scope, they do not share a static draw unit. Static draw units reference bake-local texture uses, placement assumptions, and UV transforms. They do not bind physical atlas pages, renderer texture refs, GPU IDs, or raw texture pixels.

Static object partitioning should make those axes explicit instead of hiding them inside one string key. The expected axes are material/render compatibility, source ownership, sort policy, visibility policy, and capacity slicing. Today, outdoor opaque and alpha-test objects mostly exercise material/render and capacity axes. Later, generated `outdoor-detail` foliage exercises alpha-test/cutout under the same opaque/depth-writing model; true blended material support exercises object/part sortable draw units; env-cell statics exercise visibility keys such as resident-cell or visible-cell membership.
...
Current landblock LoD is domain residency radii, not a mesh LoD tier. `SceneResourceInterest` carries `terrain`, `buildings`, `detail`, and `envCells` radii for outdoor coverage, while dungeon/interior destinations select an owning landblock pack plus a current env-cell focus. `deriveOutdoorSceneInterest` clamps outdoor buildings and detail so they do not exceed terrain coverage, and product planning turns demand into concrete landblock products. Outdoor detail should resolve generated scenery first, because foliage is the main alpha-test/cutout proof for the domain; broader explicit-object outdoor detail can trail until evidence requires it.
...
Static landblock baking should produce renderer-ingestible output after commit. Current code validates that a static draw unit is wider than a material family: it must include logical texture-use bindings, shader/binding layout, render state, sampler state, pass/order class, sort policy, visibility policy, and capacity-bounded material-table partitions. Physical atlas pages and rects remain placement-table concerns owned by the texture/atlas manager and mirrored by the renderer.

Opaque and alpha-test/cutout static object draw units are order-independent relative to each other because they write depth. They can compact by material/render compatibility, texture-role layout, batchable ownership scope, visibility scope, and capacity; object identity should remain metadata unless the partition policy needs it as a hard grouping key. True alpha-blended/translucent/inverse-alpha/additive draw units are order-dependent and should be emitted at object/part granularity with sort center/bounds metadata. The renderer then draws them after depth-writing passes, sorted back-to-front by renderer-local camera distance. Triangle-level transparent sorting is intentionally out of scope unless later evidence proves object/part sorting is insufficient.
