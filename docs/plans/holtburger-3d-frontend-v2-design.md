...
- Alpha-test/cutout surfaces are depth-writing discard materials and can participate in opaque-pass static batching. In the table-capable static-object path, the draw-unit material table carries the alpha-test threshold per material entry. The current pre-table V2 static-object renderer is still single-binding, so the alpha threshold is temporarily a draw-unit-wide renderer-binding fact until Phase 11E4A3 lands. True alpha-blended/translucent/inverse-alpha/additive surfaces are different: they need typed blend/depth state and sortable transparent submissions, so they must not be folded into the same order-independent compacted batches as opaque/cutout geometry.
