...
Opaque and alpha-test/cutout static object draw units are order-independent relative to each other because they write depth. They can compact by material/render compatibility, texture-role layout, batchable ownership scope, visibility scope, and capacity; object identity should remain metadata unless the partition policy needs it as a hard grouping key. Alpha/translucent blended static-object coverage now emits renderer-ingestible object/part draw units with render-state facts and sort center/bounds metadata; additive, alpha-additive, inverse-alpha, and inverse-alpha-additive remain evidence-gated and render-deferred until a concrete static-world target justifies the extra blend behavior. Blended draw units are object/part sortable and should be drawn after depth-writing passes, sorted back-to-front by renderer-local camera distance. Triangle-level transparent sorting is intentionally out of scope unless later evidence proves object/part sorting is insufficient.
