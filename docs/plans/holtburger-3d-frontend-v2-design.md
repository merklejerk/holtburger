...
- Indexed/paletted sampling is data sampling, not color sampling. Index pages and palette pages should be typed atlas families (`R8UI`, `R16UI`, and palette `RGBA8`) with nearest/no-mip exact lookup policy; they should not be collapsed into prepared RGBA color surfaces.
