...
Opaque and alpha-test/cutout static object draw units are order-independent relative to each other because they write depth. They can compact by material/render compatibility, texture-role layout, batchable ownership scope, visibility scope, and capacity; object identity should remain metadata unless the partition policy needs it as a hard grouping key. Blended/static alpha coverage is currently an audit/deferred path: preserve the source flags and report nonzero transparent/additive coverage, but do not emit renderer-ingestible transparent draw units until concrete static-world evidence justifies the work. If reopened, blended draw units should be emitted at object/part granularity with sort center/bounds metadata and drawn after depth-writing passes, sorted back-to-front by renderer-local camera distance. Triangle-level transparent sorting is intentionally out of scope unless later evidence proves object/part sorting is insufficient.
