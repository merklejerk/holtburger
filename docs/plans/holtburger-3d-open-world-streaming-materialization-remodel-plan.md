...
- [x] Introduce a minimal artifact runner before browser harness settlement, so terrain does not re-create `StaticCoordinator` continuations.
...
- Browser harness comparison: radius-1 replacement terrain settled with 9 terrain commits, 20 terrain draw units, renderer `error: null`, max renderer delta about 42.8 ms, one 58 ms long task, and no frame gaps over 50 ms or 100 ms. This materially avoids the legacy severe blackout profile from the investigation worksheet.
...
- Verification note: the captured browser harness log at `/tmp/holtburger-phase26-terrain-harness.log` had no matches for `terrain draw unit`, `terrain-debug-flat`, `rendered with terrain-debug-flat`, or `[holtburger-3d]`.
...
### Phase 43: Post-Cutover Audit Closeout And Triage Expansion

Deliverables:

- Close the previous "final verification" phase as an audit and steering checkpoint instead of pretending all remodel debt is gone.
- Reconcile the current implementation against the [open-world streaming stutter investigation worksheet](./holtburger-3d-open-world-streaming-stutter-investigation-worksheet.md), which remains the north star for worker ownership, loose readiness, direct diagnostics, and hard deletion of vestigial concepts.
- Add concrete remediation phases for the remaining implementation gaps discovered after Phases 36-42.

Acceptance criteria:

- The plan explicitly links the worksheet as the target design source for every follow-up phase.
- Current code drift is mapped to follow-up phases with file-level anchors, not broad bug symptoms.
- Phase 43 no longer claims to be final hard cutover while known target-shape gaps remain.
- Follow-up phases name the owning state authority for each edit: policy, owner claims, placement/page reservation, worker atlas layout, worker page build, committed page map, renderer apply, diagnostics, or harness.

Task checklist:

- [x] Reframe this phase from final verification into a steering closeout.
- [x] Ground the remaining work in the worksheet north stars instead of bug-expression triage.
- [x] Audit the current texture placement/layout/page-build shape against the target split.
- [x] Add implementation phases for the remaining design gaps.
- [x] Keep hard cutover/deletion as a later phase after those gaps are corrected.

Decisions and course corrections:

- Phase 43 is not the hard-cutover finish line. It closes the first remodel pass and opens a post-cutover remediation sequence because the remaining work is architectural, not only cleanup.
- Current production browser composition creates `WorkerPoolOpenWorldObjectVisualAtlasBuilder` through `create-browser-runtime.ts`, so atlas source preparation and layout are worker-capable in the browser path. The direct atlas builder is still used by the worker entrypoint and tests.
- Drift to fix: `texture-residency/placement/material-texture-placement-plan.ts` still owns too much orchestration. It groups by bucket, retains claims, awaits atlas layout, creates pages, reserves page builds, creates bake-facing placements, and emits page-build inputs in one function chain. That makes the split between placement authority, layout authority, and page-build authority harder to reason about than the worksheet target.
- Drift to fix: `texture-residency/atlas-build/object-visual-atlas-builder.ts` performs texture source preparation and layout together. That can be acceptable inside a worker, but the contract should make clear whether source preparation is a layout-only dimension/format read, a reusable prepared-source artifact, or duplicated work before `page-build/direct-page-builder.ts` prepares the same sources again for pixel materialization.
- Drift to fix: diagnostics are now replacement-native in `systems/open-world-streaming/diagnostics/contracts.ts`, but the surviving `compatibilityShims` field and any harness/UI consumers that still depend on `RuntimeDiagnosticsReport` need an explicit final classification: durable diagnostics export adapter, legacy-edge shim, or deletion.
- Do not turn diagnostics into a design driver. The next phases start from the worksheet target shape and current code authority boundaries, then update diagnostics to report that truth.

### Phase 44: Worker Texture Artifact Contract Split

North-star check:

- The [worksheet texture split](./holtburger-3d-open-world-streaming-stutter-investigation-worksheet.md) is authoritative for this phase: source preparation, layout search, packing, guttered blits, and page materialization are worker-owned transforms; browser/main-loop authority is placement reservation, owner currentness, renderer upload, and diagnostics.

Owning authority:

- Worker atlas layout, worker page build, placement/page reservation, and owner claims.

Deliverables:

- Refactor `texture-residency/placement/material-texture-placement-plan.ts` so its public contract reads as reservation orchestration, not a grab bag of bucket grouping, atlas build, page allocation, bake placement, and page-build input synthesis.
- Split the atlas-build contract in `texture-residency/atlas-build/object-visual-atlas-builder.ts` into explicit worker-owned outputs:
  - source facts required for layout validation: entry id, format, width, height, and any source-stability facts needed to prove bucket compatibility;
  - placement layout: virtual layout-local pages and rects;
  - stage timings that distinguish source fact preparation from layout search.
- Decide whether page-build should reuse source facts from atlas layout or intentionally prepare sources again for pixel materialization in `texture-residency/page-build/direct-page-builder.ts`. If duplication remains, document and test why it is structurally necessary.
- Keep direct builders worker-internal or test-only. Production browser composition must continue to inject worker-backed builders from `create-browser-runtime.ts`.
- Update tests around `material-texture-placement-plan.test.ts`, `object-visual-atlas-worker-client.test.ts`, `object-visual-texture-placement-plan.test.ts`, and `page-build.test.ts` so they assert the authority split rather than old call sequencing.

Acceptance criteria:

- `material-texture-placement-plan.ts` no longer needs to know how source pixels are prepared.
- Atlas layout output names source-fact preparation separately from page pixel materialization.
- Page-build input contains only immutable page/materialization facts and never depends on mutable main-thread prepared-source cache objects.
- Production browser path cannot accidentally instantiate a direct atlas or direct page builder on the main thread.
- Focused texture placement, atlas-build, and page-build tests pass.
...
- The target is explicit replacement-owned bucket policy, not ad hoc terrain/object branches and not ignored legacy placement fields.

Owning authority:

- Texture placement policy and material/source identity facts.

Deliverables:

- Audit every producer of `TexturePlacementIntent.placementPolicy` that reaches `createMaterialTexturePlacementBucketKey(...)`.
- Confirm terrain, static object, env-cell, generated scenery, runtime-authored dynamic, and static-authored dynamic paths use `content-stable` versus `owner-specific` from real source facts.
- Delete any remaining policy field that is set but ignored by the replacement residency path.
- Collapse any terrain-only or domain-only bucket special case into the shared `static-domain` / `static-owner` / `runtime-owner` policy model, or document why it is a measured exception.
- Add tests that prove bucket scope from source facts, not from legacy domain folklore.

Acceptance criteria:

- `material-texture-placement-policy.ts` remains the only policy-to-bucket resolver for replacement material textures unless a phase records a new explicit authority.
- No production replacement code chooses bucket scope by checking "terrain", "generated scenery", or "env-cell" directly when source stability facts are available.
- Static-authored content-stable textures share static-domain buckets across compatible owners.
- Owner-specific generated, tinted, placement-specific, and runtime-authored sources use owner-scoped buckets and fail loudly if mislabeled.
- Focused placement-policy and object/terrain/env-cell material planning tests pass.
...
- Material coverage remains evidence only. Replacement readiness must report whether geometry is renderable, pending texture dependencies, unsupported source material, renderer capability deferral, or a pipeline bug.

Owning authority:

- Material readiness diagnostics, static bake evidence, renderer capability, and scene commit install readiness.

Deliverables:

- Audit `static/objects/bake/static-object-material-coverage.ts` as an evidence producer and verify every consumer treats it as `sourceEvidence`, not final renderability truth.
- Trace readiness from `object-visual-material-planner.ts`, structured interior material planning, terrain material construction, bake output, scene commits, renderer binding readiness, and `diagnostics/contracts.ts`.
- Close or classify remaining flat-color/fallback output by direct cause:
  - missing material recipe;
  - unsupported source data;
  - deferred renderer feature;
  - pending/failed texture dependency;
  - valid intentional flat-color material.
- Add direct diagnostics for "intentional flat color" only if the renderer needs to distinguish it from missing texture/material failure.
- Update UI/harness consumers to show direct readiness counts without adding coverage-shaped shims.

Acceptance criteria:

- Terrain and static object flat-color rendering can be explained from direct readiness diagnostics without reading static coverage reports manually.
- No readiness issue kind preserves old broad coverage identity as the canonical issue.
- Missing-not-in-flight bindings are reported upstream as replacement diagnostics.
- Tests prove material coverage evidence maps into direct readiness facts and cannot be mistaken for successful rendering.
- Browser harness outputs enough direct readiness information to debug flat color without adding a legacy diagnostic projection.
...
- Migrate direct contracts, shim legacy. Diagnostics are not exempt. Legacy report parity is not evidence that the replacement model is correct.

Owning authority:

- Diagnostics export plumbing, harness composition, and UI migration edges.

Deliverables:

- Classify every surviving `RuntimeDiagnosticsReport` and `openWorldStreaming.compatibilityShims` consumer as direct migration, deletion, legacy-edge shim, or durable adapter.
- Migrate `BrowserPipelineHarness.svelte`, `browser-pipeline-harness.mjs`, and `BrowserDisplay.svelte` consumers to direct `OpenWorldStreamingDiagnosticsSnapshot` fields where they are intended to survive.
- Delete compatibility fields that exist only to make old static coordinator or texture manager snapshots look alive.
- If a shim remains, move it to the edge that owns the blocked legacy consumer and record its deletion trigger in this phase.
- Ensure diagnostics categories name replacement behavior: owners, artifacts, bucket lanes, claims, page builds, commits, readiness, stale rejection, and frame/yield behavior.

Acceptance criteria:

- No replacement-internal diagnostic field exists solely because an old report, panel, or test expected it.
- `compatibilityShims` is either deleted or proven to be a temporary edge-owned report with a named consumer and deletion phase.
- Harness summaries use direct replacement facts for settlement/readiness.
- UI panels do not recreate `staticOverview`, static coordinator timing, texture manager mutation lanes, or old material coverage categories.
- Focused diagnostics/UI/harness tests pass.
...
- Owner-indexed residency is the source of truth. Page deletion and repack are policy decisions, not eager side effects and not hidden leaks.

Owning authority:

- Owner claims, committed page map, texture commits, and renderer apply.

Deliverables:

- Convert Phase 41's ownerless diagnostics into an explicit first-cut reclamation policy or record a measured deferral with pressure thresholds.
- Define when ownerless resident pages are cached, reclaimable, scheduled for renderer removal, or retained for reuse.
- Implement texture removal commits if pressure policy says a page should leave the renderer, using the existing `pageRemovals`/`removedTextureRefIds` commit/applier path.
- Ensure `releaseTextureOwner(ownerId)` is cheap and idempotent while still producing enough state for reclamation diagnostics.
- Add tests for owner release, ownerless resident classification, reuse, and removal commit emission.

Acceptance criteria:

- Ownerless resident pages cannot sit in an ambiguous state.
- Renderer-removal commits are either implemented behind a direct policy or explicitly deferred with measured thresholds and diagnostics.
- Diagnostics report ownerless entries/pages by retained state and any pending reclamation/removal work.
- No lease/pin vocabulary is introduced into replacement texture residency.
- Focused claim-registry, texture commit, and renderer applier tests pass.
...
- Worker slowness is acceptable when the browser stays responsive. Frame budgets are a measured fallback for main-loop offenders, not an architecture.

Owning authority:

- Artifact scheduling, renderer apply, harness diagnostics, and benchmark evidence.

Deliverables:

- Rebaseline `dc58` and `da55`, radius 1, all domains after Phases 44-48.
- Compare terrain-only, terrain+generated-scenery, terrain+env-cells, and all-domain runs against the worksheet baseline.
- Attribute any remaining long tasks to scheduling, source delivery/deserialization, atlas layout, page-build acceptance, renderer upload/apply, scene commit install, or harness settle criteria.
- Add frame budgets only for proven browser-main-loop CPU offenders after deletion/off-thread movement is not sufficient.
- Update future cleanup tasks based on measured evidence, not speculative budget knobs.

Acceptance criteria:

- Harness outputs include max frame gap, long task counts, runtime tick timing, renderer frame timing, static task diagnostics, texture residency/page-build diagnostics, and material readiness summary.
- Generated scenery no longer recreates the original 1s+ texture mutation blackout.
- Dense `da55` evidence is recorded, even if it exposes a separate env-cell/renderer draw issue.
- Any new budget/yield is tied to a named measured main-thread stage and has a focused test or harness assertion.
- The plan records remaining performance debt with exact benchmark commands and output paths.
...
- Issue 57.3: The old texture page viewer modal was removed during hard cutover, leaving the replacement texture page list without click-through inspection. Classification: replacement-native inspector gap, not a legacy modal restoration. Root cause: Phase 16 deleted the old `TextureAtlasPageInspectorModal` and `TextureAtlasPageInspectionSnapshot`, while the replacement controller only retained a stub `createAtlasInspectionSnapshot()` that always returned `missing`. Fix: replaced the stub with `createTexturePageInspectionSnapshot()` backed by the texture claim registry, exposed a runtime `RuntimeTexturePageInspectionSnapshot`, retained accepted page pixels and binding rects from texture commits for inspection, and added a `TexturePageInspectionModal` opened by click/keyboard from texture page rows. The modal restores the interactive atlas-style preview with pan/zoom, fit, bounds overlay, and click-to-select placement while also showing direct replacement facts: page state, bucket domain/purpose/scope/key, entry state, owners, bindings, page class, source key, and texture key. Verification: `npm run check`, focused texture-claim-registry and controller tests, `npm run lint`, and terrain-only harness `/tmp/holtburger-phase57-issue3-texture-preview.json` passed; harness diagnostics showed 3 bucket rows, 84 entries, 31 resident pages, and the first page retained 4 entries. Remaining caveat: harness coverage proves the residency/page source data, while the interactive canvas path is covered by Svelte/TypeScript checks rather than a browser click screenshot. Status: fixed, awaiting review/commit.
- Issue 57.4: The texture page inspection modal restored the old inspection capability but the first replacement-native layout was too dense to inspect or interact with comfortably. Classification: diagnostics/UI ergonomics gap, not a core texture model issue. Root cause: the modal attempted to show preview, page summary, entry list, and selected-entry details in one stacked right rail; after entry dedupe, full owner/binding lists can be long enough that this shape becomes actively misleading. Fix: enlarged the modal, made the texture preview the dominant workspace, moved right-rail details behind `Page`, `Entries`, and `Selected` tabs, and made entry selection switch to the selected-entry pane. Verification: `npm run check` and `npm run lint` passed. Remaining caveat: the selected-entry pane still renders full owner and material-binding lists inline; after Issue 57.5 proved large shared entries are expected, the next UI polish should summarize owner/binding counts first and put the full list behind expansion/copy. Status: fixed, awaiting review/commit.
- Issue 57.5: Texture page inspection showed repeated-looking terrain textures across many pages and sometimes within a page, and follow-up screenshots showed only one, then two, landblocks rendering terrain textures while neighboring landblocks fell back to flat color. Classification: texture residency contract bug plus placement/page-reservation/readiness gap. Root cause 1: texture entry identity was over-composed as `bucketKey + textureKey + pageClass + purpose + sourceKey`, and `sourceKey` in the placement path was incorrectly populated from `String(intent.itemId)`. For terrain, `itemId` is a material binding id that includes owner/resource identity, so canonical textures with the same `textureKey` were split into separate entries. Fix 1: `TexturePlacementIntent` now carries an explicit prepared `sourceKey`; terrain and object-visual placement pass direct prepared source keys; material placement forwards `intent.sourceKey`; and the claim registry now keys entries by `bucketKey + textureKey` while treating `pageClass`, `purpose`, and `sourceKey` as loud consistency assertions. Root cause 2: the placement path planned atlas pages for every touched entry and `reserveAtlasLayoutPage()` called `textureClaims.createPage(...)` for every layout result. The registry did not retain enough per-entry placement facts to answer "this entry is already assigned to a usable page." Fix 2: texture pages now retain direct placement facts (`pageId`, `textureRefId`, dimensions, and per-entry rects), the registry exposes reusable entry placements for `building`, `resident`, and resident-retained `reclaimable` pages, and material placement splits touched entries into reusable versus unplaced before atlas planning. Root cause 3: in-flight page reuse skipped additional page builds, but the already-dispatched page-build request only contained the first owner's binding ids. Settlement therefore emitted resident binding updates from stale worker-output bindings and never resolved later bindings that had reused the in-flight page. Fix 3: page-build settlement now accepts the page token, then asks the claim registry for current binding placements for the accepted page so the texture commit resolves every current binding for the page entries. Root cause 4: resident/reclaimable page reuse returned bake-facing placements but no renderer-facing binding updates, so later landblocks reused an existing atlas page while their renderer bindings stayed pending and terrain fell back to flat color. Fix 4: material placement now emits binding-only texture commits for resident and reclaimable page reuse, layer commits carry those texture commits, and the controller applies them before installing draw units. Renderer terrain-material diagnostics were added to prove fallback mode and non-resident binding counts from harness output. Verification: focused Vitest passed for texture claim, placement, page-build, commit applier, controller, layer runner, and renderer diagnostics tests; `npm run check` and `npm run lint` passed; terrain-only browser harness `/tmp/holtburger-phase57-issue5-binding-reuse-terrain.json` passed with `errorMessage: null`, zero material readiness issues, 6 resident pages, 6 queued/committed page builds, and duplicate-entry scan `[]`; all-domain browser harness `/tmp/holtburger-phase57-issue5-binding-reuse-all-domain.json` passed with `errorMessage: null`, 20 ready artifacts, zero material readiness issues, 12 resident pages, 12 queued/committed page builds, 7 buckets, 58 entries, max 4 owners/bindings per entry, and duplicate-entry count `0`; follow-up browser harness `/tmp/holtburger-phase57-da55-binding-only-commits.json` passed with `errorMessage: null`, 55 layered terrain draw units, zero flat fallbacks, zero non-resident terrain bindings, and 96 accepted/committed texture page builds. Remaining caveat: placement reuse is intentionally incremental. It prevents one entry from being uploaded on multiple pages, but it does not repack already assigned pages into a globally minimal layout. Status: fixed, awaiting review/commit.
- Issue 57.6: Clicking a placement in the texture page inspector canvas highlighted the canvas rect but did not update the selected entry details or entry-list selection. Classification: replacement-native inspector UI state bug. Root cause: `TexturePageInspectionModal` kept `selectedPlacementBindingId` and `selectedEntryId` as independent state; canvas hit-testing only changed the binding id, while entry-list clicks only changed the entry id. Fix: route canvas and entry-list selection through local helpers that map placement binding ids to entries and keep the selected rect and selected entry synchronized. Verification: `npm run check` and `npm run lint` passed. Remaining caveat: the preview placement DTO still carries `bindingId` only; the modal derives entry membership from `snapshot.entries`, which is adequate for the current inspection contract. Status: fixed, awaiting review/commit.
- Issue 57.7: Texture page rows and the texture page inspector lacked a compact packing/assignment metric, making sparse single-entry pages hard to distinguish from dense pages. Classification: replacement-native diagnostics/UI gap. Root cause: texture page snapshots retained page dimensions and placement rects internally but did not expose assigned pixel counts through the resource overview or inspector contracts. Fix: texture claim page records now report assigned entry pixel count and total page pixel count, the runtime overview and inspection snapshot expose assigned pixel ratios, resource rows display `% used`, and the inspector page panel displays assigned pixels as percent plus raw assigned/total counts. Verification: focused texture claim/controller Vitest, `npm run check`, and `npm run lint` passed. Remaining caveat: assigned pixels are the sum of per-entry rect areas and assume atlas entry rects do not overlap; overlapping rects would be a placement invariant bug, not a UI metric problem. Status: fixed, awaiting review/commit.
- Issue 57.8: The assigned-pixel metric exposed many compatible textures spread across sparse pages. Classification: texture placement policy gap, not a renderer or inspector bug. Investigation: terrain-only/all-domain harness `/tmp/holtburger-phase57-da55-packing-investigation.json` passed with `errorMessage: null` but reported 96 resident pages, 64 single-entry pages, 72 pages at or below 20% assigned pixels, and average page usage about 16.7%. Terrain color had 7 entries across 5 pages, with 4 single-entry 2048x2048 pages at 6.25% usage; terrain mask had one 7-entry page at 43.75% plus one single-entry page at 6.25%; generated scenery base color had 66 entries across 46 pages, 42 of them single-entry. Root cause: material placement reused exact entries already assigned to pages, but unplaced compatible entries were sent to fresh atlas layout and `textureClaims.createPage(...)`; existing resident pages with free space were never presented as locked pages for insertion. Fix: texture pages now retain the page-build facts needed to rebuild every assigned entry, texture residency exposes resident-page insertion candidates, and material placement calls `planAtlasPageInsertion(...)` before fresh-page allocation. Inserted entries are added to the existing page without moving old rects, the resident page is reserved for an explicit rebuild, bake-facing placements point at the existing texture ref, and fallback fresh allocation only handles entries that could not be inserted. Verification: focused texture claim/material placement/page-build Vitest passed, broader static layer/controller Vitest passed, `npm run check`, `npm run lint`, and browser harness `/tmp/holtburger-phase57-da55-page-insertion.json` passed with `errorMessage: null`; resident pages dropped from 96 to 27, single-entry pages from 64 to 3, pages at or below 20% from 72 to 8, average assigned-pixel usage rose from about 16.7% to about 31.7%, terrain color collapsed from 5 pages to 2, terrain mask from 2 pages to 1, and renderer diagnostics still reported 55 layered terrain materials with zero non-resident texture bindings. Remaining caveats: insertion currently targets resident pages only, not building or reclaimable pages; resident insertion rebuilds the touched page and slightly increased accepted page-build tasks in the measured harness; the placement path still asks the atlas builder for source facts before attempting insertion, so a future cleanup can split source-fact preparation from fresh-page layout. Repacking/moving already placed entries remains a separate measured policy decision and was not smuggled into this fix. Status: fixed, awaiting review/commit.
- Issue 57.9: Static-authored dynamic butterfly textures still appeared as repeated identical wing entries after Issue 57.8, with inspector texture keys containing `dynamic-visual-resource`, landblock, cell, template, setup, and material path facts. Classification: texture identity bug in dynamic visual planning, not atlas packing. Root cause: `createRuntimeAuthoredDynamicTextureIdentity(...)` was used for every dynamic visual texture requirement. For static-authored dynamic visuals the placement policy already said `static-domain` plus `content-stable`, but the texture key used `src=runtime` with `dynamicTextureSourceId`, making one prepared render surface become many logical texture entries. Harness proof before the fix showed `prepared-render-surface-texture-use:06006270:rgba-color` mapping to 42 distinct texture keys. Fix: dynamic visual texture identity now derives its texture source key from `TexturePlacementPolicy.sourceStability`: `content-stable` prepared render surfaces and no-replacement palettes use canonical material source keys, while `owner-specific` runtime-authored textures keep runtime-scoped keys. Focused tests now prove static-authored dynamics with the same render surface share one canonical texture key, and runtime-authored dynamics with the same source facts remain runtime-owner-scoped. Runtime-authored audit: current production runtime-authored dynamics always use `runtime-object-material` with `runtime-owner` and `owner-specific/runtime-customized`; there is no current content-stable runtime-authored sharing path to collapse. Verification: focused dynamic/policy Vitest, `npm run check`, `npm run lint`, and browser harness `/tmp/holtburger-phase57-da55-static-authored-dynamic-texture-identity.json` passed with `errorMessage: null`; generated-scenery runtime-keyed entries dropped to zero, `06006270` collapsed to one render-surface texture key with 49 bindings/owners, generated-scenery base-color pages dropped to two pages, material readiness issues stayed at zero, and renderer diagnostics reported 44 dynamic visual resources, 44 dynamic instances, zero skipped dynamic submissions, and zero non-resident terrain bindings. Remaining caveat: content-stable dynamic palette textures with replacement ranges still need palette replacement fingerprints before they can be assigned canonical palette texture identity without weakening the key. Status: fixed, awaiting review/commit.
- Issue 57.10: Static-authored dynamic entities reached renderer draw submission with `texture-rgba` material payloads but skipped draws because their texture bindings stayed pending. Classification: texture residency workflow split and upstream readiness gap, not a renderer draw bug. Root cause: runtime entities used the lower-level object-visual placement planner directly enough to get bake-facing placement snapshots and page-build requests, but their orchestration did not share the static layer texture-residency workflow that applies renderer-facing binding commits returned by reservation reuse. A second gap appeared after moving detection upstream: dynamics can reuse a page that is already `building` for another owner, so their own reservation has no page-build request to await even though their baked render-part bindings are not resident yet. Fix: introduce `OpenWorldTextureResidencyService` as the single path for material/object-visual placement reservation, renderer-facing reservation texture commits, owner release, and binding-residency inspection. Terrain, outdoor object, env-cell, runtime-authored dynamic, and static-authored dynamic materialization now call that service instead of each owning ad hoc claim/commit plumbing. Dynamic prep now validates the exact baked render-part texture bindings before publishing a ready resource, waits for waitable `page-building` residency, and fails upstream for non-waitable unresolved binding states. Renderer hot-path skipped-draw console output and `skippedDynamicSubmissions` fallback telemetry were removed; the renderer remains defensive but is not the diagnostic owner. Verification: `npm run check`, focused renderer/controller/static-runner/runtime-entity/texture-claim Vitest, `npm run lint:ts`, structural grep confirmed production raw placement calls are confined to the texture residency service and placement internals/tests, structural grep confirmed the renderer skip telemetry symbols are gone, and browser harness `/tmp/holtburger-phase57-issue10-upstream-dynamic-texture-readiness-2.json` passed with `errorMessage: null`, 44 ready dynamic visuals, zero runtime prep failures, zero material readiness issues, renderer `dynamicVisualResources: 44`, renderer `dynamicInstances: 44`, and no renderer `skippedDynamicSubmissions` field. Remaining caveat: source resolution and bake preparation remain layer-specific because their input domains differ; that is not a residency split. Status: fixed, awaiting review/commit.
- Issue 57.11: Follow mode appears to flicker during movement, reanchoring appears ineffective, out-of-interest landblocks remain visible, and new landblock loading is slow. Classification: static interest cleanup contract bug plus performance investigation. Current-code evidence: `OpenWorldStreamingController.updateStaticInterest(...)` immediately updates the renderer anchor with `setStaticRenderAnchorLandblockId(...)`, then starts async static materialization; `#evictStaticOwners(...)` only removes static-authored dynamic children, evicts owner tokens, and releases texture residency owner claims. It does not clear renderer static layers with null payloads and does not call `StaticSceneQuery.retainLayerOwners(...)`, even though the renderer layer setters already support `payload: null` and `StaticSceneQuery` has owner-retention pruning. This leaves renderer resources and query/index state outside the owner-retention authority and matches the observed "scene keeps extending" behavior. Harness evidence: browser harness `/tmp/holtburger-follow-investigate.json` for landblock sequence `0xda55ffff,0xda56ffff,0xda57ffff` timed out before the first step settled; diagnostics showed 57 requested static tasks, 9 applied scene commits, 48 pending commits, 1 active task, 74 current owners, 44 evicted owners, 1,781 browser long tasks totaling about 209s before static ready, and terrain `resolve-source` stages in recent tasks around 9-36s. Interpretation: the cleanup/reanchor bug is proven by code structure; the slow-loading complaint is also real but is primarily source-resolution/materialization throughput, not renderer upload. Proposed structural fix: make the retained `LayerOwnerKey` set from `planStaticDemand(...)` the single authority for static interest pruning, and update renderer layers, `StaticSceneQuery`, runtime children, owner tokens, and texture claims from the same retained-owner decision. Do not add renderer hot-path fallbacks or diagnostics-shaped cleanup. Proposed follow-up after cleanup fix: re-run a follow/manual landblock sequence harness and then investigate source-resolution fanout/caching/concurrency if settlement remains slow. Status: investigated; fix not started.
...
- Issue 57.3 restores the old interactive viewer affordance without restoring the retired TextureManager contract. The replacement inspector retains accepted commit previews directly from texture residency, so the canvas is current only after page-build settlement accepts the commit.
- Issue 57.5 split duplicate-looking texture evidence into two different facts: shared entries correctly accumulate many owner and material-binding claims, while duplicated physical page uploads remain a separate placement reuse problem. Do not re-expand entry identity to hide the fan-out; the next fix should persist and reuse page placement facts for existing entries.
- The canonical entry identity is now `bucketKey + textureKey`. `pageClass`, `purpose`, and `sourceKey` are consistency assertions, not dedupe discriminators. If these assertions trip, the underlying texture identity or bucket policy is wrong and should fail loudly instead of creating another entry.
- Issue 57.5 page-placement reuse is now owned by texture residency placement/page reservation, not diagnostics. Building pages are reusable because a real upload is already in flight; arbitrary planned pages are not reusable because they may be failed/noop leftovers without an active build.
- Issue 57.5 deliberately keeps incremental placement instead of repacking resident pages. Repacking remains a future measured policy decision because it implies renderer removals/reuploads and would conflict with the Phase 54 reclamation deferral without fresh pressure evidence.
- Issue 57.5 binding readiness must be derived from current registry state at page-build settlement, not from worker-output binding ids. Worker output owns pixels and rect echoing for the original request; texture residency owns current owner/binding membership.
- Issue 57.5 resident and reclaimable page reuse must also emit renderer-facing binding-only texture commits. Bake-facing placement reuse alone is insufficient because the renderer owns a separate resident binding table and must not reach back into texture residency during draw.
- Issue 57.8 distinguishes insertion from repack. Inserting new entries into existing free space is aligned with bucket placement ownership and should be attempted before fresh-page allocation; moving already placed entries across pages is a higher-risk compaction/repack policy and remains deferred until measured pressure justifies renderer removals/reuploads.
- Issue 57.8 existing-page insertion is restricted to resident pages with complete page-build facts. Building-page insertion would require mutating or superseding an in-flight worker request, which is a different protocol problem; reclaimable-page insertion would need explicit resurrection policy. Keep those out until measurement says the added lifecycle complexity is worth it.
- Issue 57.9 makes `TexturePlacementPolicy.sourceStability` the authority for dynamic visual texture keys. Authoring source is not enough: content-stable sources may share canonical texture identity, while owner-specific sources must stay runtime-scoped even if they reference the same DAT render surface.
- Issue 57.9 deliberately fails content-stable dynamic palette replacements that lack fingerprinted replacement identity instead of inventing a weak canonical key. Add asset-reader-backed palette fingerprinting to dynamic texture planning if static-authored dynamic palette replacement sharing becomes required.
- Issue 57.10 makes texture residency a shared service boundary instead of a per-materializer convention. Static and dynamic paths may produce different placement intents and bake inputs, but raw placement reservation plus renderer-facing reservation texture commit application must stay centralized under `texture-residency`.
- Issue 57.10 preserves owner/run currentness at the service boundary. Applying texture commits inside a shared service is acceptable only because callers pass the same freshness predicate they use for artifact acceptance; stale placement work must not mutate renderer binding state.
- Issue 57.10 assigns meaningful dynamic texture readiness detection to runtime prep, not the renderer hot path. Renderer-side skips are a defensive rendering behavior during loose commit ordering; upstream systems must decide whether unresolved bindings are waitable, failed, or impossible before marking a dynamic visual ready.
- Issue 57.11 should restore owner-retention cleanup before tuning source-resolution throughput. Cleanup is a contract bug with a clear authority violation; throughput is a measured performance problem that needs fresh attribution after stale renderer/query layers stop accumulating.
- Issue 57.11 reanchor/follow fixes must not make the renderer pull placement, interest, or owner state at draw time. The renderer can accept explicit layer removals and anchor changes, but streaming ownership remains in open-world materialization.
