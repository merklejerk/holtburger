...
Issue 5 is closed following user acceptance. Issues 6 (camera obstacles) and 7 (precise-jump latency) are jointly scoped below. The previously recorded issue 3a follow-up remains separate.

## Issue 6 — camera boom passes through solid entities

Status: **Resolved — implemented and accepted by the user.** Original investigation below is retained as evidence; resumed implementation records the code and validation.

Report: the boom passes through doors, hiding the player behind the door and exposing the next room.

### Confirmed code-path evidence

- `client/camera.rs:367–426`: `ClientCameraSceneInput::capture` publishes the local player's body facts and an `Arc<CollisionScene>`. It does not capture other entity collision shapes. `CollisionScene` (`world/spatial/collision.rs:1005`) contains landblock assets and their static spatial index, not live entity bodies. “Static entity” and “static environment geometry” are different data categories here; a stationary door is still a live entity.
- `kinematic_boom.rs` delegates placement/separation and movement to `settle_free_sphere_with_policy` and `solve_free_sphere`, and reach obstruction to `sweep_static_sphere_with_policy` (line 1405). `free_sphere.rs:427–477` gathers environment contacts and restrictions only. `PhysicalCollisionFilter::ALL` selects all supported environment domains; it cannot supply the absent entities. This is a missing query input/path, not evidence of a bad door mesh or radius.
- The existing combined `CollisionScene::sweep_hard_sphere` accepts `HardEntityShape` with placed geometry and spatial membership (`collision/static_sphere_sweep.rs:80,123`). Reuse is available for continuous obstruction; merely switching the one reach sweep would still leave the boom's placement/separation paths environment-only.

### Retail evidence

`acclient.c:138810–138919`, `SmartBox::update_viewer`, makes a transition from the player pivot to the desired camera position, initializes object flags with 92 (`0x5c`, including viewer and path-clipped), and supplies a camera sphere. `acclient.c:304609–304684`, `CPhysicsObj::FindObjCollisions`, processes object collision and explicitly skips creatures for viewer queries. ACE corroborates the names and predicate in `Physics/PhysicsObj.cs:380–397` and `Physics/ObjectInfo.cs`. Ethereal objects follow nonblocking collision handling. Retail therefore does not omit all live objects, nor does it simply block on every character. The camera sphere is initialized to 0.3 m at `acclient.c:139301–139305`; our projection-derived clearance is a separate existing policy, not the demonstrated cause.

### Correction direction and remaining work

Give camera collision a coherent view of eligible live solid entity geometry alongside environment geometry, reusing the current pose-aware collision publication from issue 4. Keep obstacle eligibility in shared world semantics and boom comfort/reach behavior in core. Do not add door-specific meshes or renderer picking to physical camera placement. Cover every boom query path, including stationary overlap when a door closes, clearance changes, and recovery, rather than only clipping desired reach. Exclude the target and retail-ineligible creatures/nonblocking objects; verify the exact remaining eligibility rules before implementation.

The input omission is established by source inspection; no live login or empirical door reproduction was needed to establish that structural gap. No claim of a tested fix or measured performance. Next validation should compare closed/open and moving door poses, non-door solid entities, creatures, and portal-straddling obstacles using the production camera path. Review coherent snapshot ownership and broad-phase selection before selecting the final implementation shape.

### Problem-solving scope

**Required contracts.** One camera tick must use coherent environment, entity pose/participation, and target facts. All collision-bearing operations must use the same obstacle population: initial clearance, clearance growth, radial reach, lateral travel, and overlap recovery. Preserve sphere clearance, finite work limits, portal membership, immediate obstruction response, and gradual outward recovery. A stationary camera must observe entity changes. Camera queries must never push entities, emit gameplay contacts, or change door state. Client camera and Explorer possession share the controller and must both supply the new input explicitly.

**Data distribution.** Obstacles include stationary-root objects with animated collision parts; checking root velocity or requiring a settled animation is insufficient. Prior issue 4's archive census found 90/92 pose-varying candidate setups had one or two differing collision parts; this is historical shape evidence, not a camera benchmark or live population count. Nearby membership and shape bounds should bound query work. The snapshot/index already supports multiple posed parts and portal-straddling targets. Do not assume every non-creature is static or every non-static object should be ignored: retail reaches non-static object collision too (`acclient.c:304745–304775`). Exact physics participation remains authoritative.

**Concessions.** Use published obstacle poses frozen for one camera tick; continuous time-of-impact against an independently rotating door is outside this change. Resolve newly observed overlap on the next publication/tick. Preserve bounded failure/fallback behavior when no valid camera placement exists, and do not advertise uncovered environment queries as fully proven. Missing required entity preparation must not be silently reported as complete entity coverage. This correction prevents the demonstrated local camera leak; it is not a server-side visibility/security guarantee.

**Neighborhood and reuse.** `SpatialScene` owns bodies and can seal an `EntityCollisionSnapshot`; `dynamic_index.rs` owns membership/bounds indexing and `placed_target_shapes`, which applies the already-published collision poses. Core's client camera capture currently discards those entity facts. The Explorer adapter is `apps/holtburger-3d/host/src/host_kinematic_boom_runtime.rs` (the repository now uses `host`, not the historical `src-tauri` path). Shared world query semantics should consume posed candidates; core retains boom smoothing and recovery policy; adapters only capture/pass inputs. No renderer mesh access or new animation owner is needed.

**Alternatives considered.** A door-only stop plane duplicates authored geometry and misses other solids. Adding entities only to `cast_to_reach` leaves transit and settlement inconsistent. Reusing precise-jump's selectable-surface query imports the wrong contract: `selectable_target_proof` requires `DynamicBodyActivity::Settled`, excluding moving obstacles, and a ray does not protect the camera envelope. Inserting live entities into landblock assets mixes independent lifetimes. Replacing the complete boom controller with player movement imports unnecessary support/step/contact behavior.

**Recommended shape.** Add a small world-owned, read-only obstacle query context combining environment collision with admitted entity shapes from a coherent publication. Reuse the existing entity snapshot/index and posed-shape preparation; decide camera eligibility once at the world boundary, not repeatedly in core. The context supplies combined sphere sweep and contact/separation operations to the existing free-sphere/boom algorithms. Factor their environment-only contact collection at its smallest useful seam; avoid a second free-sphere solver or a general-purpose physics framework. Keep environment-only callers explicit. Reuse hard swept-sphere narrow phase where appropriate, preserving the camera's existing uncovered-query policy rather than accidentally switching it to strict movement policy.

Camera eligibility must exclude the target, creatures, and nonblocking participation, while admitting eligible moving non-creature objects as well as stationary doors. Confirm the predicate against retail/ACE and the existing prepared collision facts before coding; do not equate “selectable,” “settled,” or “static” with camera-solid. The current general entity snapshot clones bodies and builds an index, so do not blindly rebuild it per control leg or render frame. Capture once at the producer's publication boundary and reuse within camera work; use existing membership/bounds candidate selection. Introduce a persistent cache/revision only if ownership and measured cost justify it.

**Implementation/verification order (proposed, not authorized).** First prove eligibility and combined sphere sweep/overlap with synthetic posed bodies, including a portal-straddling door and moving non-creature. Then thread a coherent obstacle input through client and Explorer boom owners and every boom query, with unchanged environment-only behavior covered by existing tests. Finally verify closed/open/closing doors and stationary-camera recovery through the real camera runtime/browser, including player visibility and camera placement on the player's side. Test creature exclusion, despawn, replacement, late preparation, clearance growth, and crowded local candidates. A synthetic eligibility/sweep prototype should precede the full wiring if either predicate or overlap response remains uncertain.

Expected scope: a focused cross-layer extension of world queries, core boom inputs, and two capture owners, plus tests. No frontend protocol change is expected. Production line count is not yet estimated accurately; reject duplicated solvers or a parallel entity registry during implementation review. Performance is unmeasured; the structural cost target is one shared capture per publication and local candidate queries, not repeated whole-population geometry preparation.

## Issue 7 — sluggish precise-jump targeting

Status: **Resolved — implemented and accepted by the user.** The user reports noticeably smoother targeting. Tuning is unchanged. The user is open to a better shared pattern for camera and prediction queries. This expands the earlier camera-only proposal; it does not establish that copying is the measured bottleneck.

### Evidence and current limits

- Frontend `client-tuning.ts` caps aim submission at 30 Hz. `client-precise-jump-session.ts` also permits only one outstanding evaluation; further pointer samples coalesce until its result arrives. Actual responsiveness includes evaluation round-trip time, not just the 33.3 ms cadence. Raising the cap cannot remove a slower outstanding request.
- `precise_jump_runtime.rs::submit` captures `world.scene.clone()` for an accepted aim, even when replacing queued work. `evaluate` then builds `EntityCollisionSnapshot`, cloning entity body records and constructing the spatial index. Commit performs fresh capture/evaluation for correctness.
- `precise_jump_prediction.rs::predict_candidate` clones the spatial scene per candidate. The current budget is six candidates and up to 160 fixed 30 Hz simulation ticks per candidate; these are ceilings, not evidence that every aim spends all 960 ticks.
- `SpatialScene::tick_physical_body_against_entity_snapshot` additionally clones frozen peer bodies on every simulated tick, marks their integration excluded, and invokes the common contact collection. Only the mover's result is published. This is a particularly relevant repeated-work boundary for measurement.
- Immutable collision geometry and collision-part pose arrays use shared references; body-record cloning is not equivalent to copying every mesh. Population-dependent allocations, membership copies, index construction, and contact preparation still need attribution.
- The existing `precise_jump_benchmark` constructs its entity snapshot outside the timed prediction loop and uses a representative player fixture. Its prediction timing alone cannot establish live capture cost or pointer-to-marker latency. Extend diagnostics to cover the missing stages rather than citing that benchmark as complete evidence.

## Joint scope — shared immutable collision queries for issues 6 and 7

### Contracts and ownership

World owns a coherent immutable obstacle publication: environment collision plus the entity identity, pose, collision-part transforms, participation facts, spatial membership, and bounds needed by actual queries. Meshes remain shared. Derive geometry/index facts once at their producer; consumers must not reconstruct full mutable entity bodies merely to query frozen surfaces. Keep the canonical body store as authority, with no second mutable registry.

Core camera and precise-jump workers retain the same publication for the duration of their work. A speculative jump owns its initial mover state and one mutable mover per candidate. Camera owns its existing boom state. Neither query changes peers or emits gameplay contact reports. Preserve the existing prediction approximation of frozen peers and the current movement/contact response; sharing obstacles does not justify making camera and character collision eligibility identical.

World supplies query-specific admission for camera, jump movement, and selectable landing surfaces from shared physical facts. Camera excludes creatures while jump movement can collide with them; moving solids can block a camera even when they are not eligible selectable landing targets. Preserve directional collision, support provenance, portal-domain admission, scales, BSP/fallback selection, and posed geometry. Share preparation and low-level queries, not these distinct behavioral policies.

Core owns publication capture/lifetime and asynchronous work scheduling. Client and Explorer adapters pass explicit inputs to the shared boom. The frontend retains aim coalescing, cadence, and presentation. No new renderer geometry path, animation owner, general scene service, or frontend protocol is expected.

Publish/capture only when demanded by an active consumer, and share a coherent capture where consumers need the same state. Exact publication granularity remains a prototype decision: compare a compact capture with existing publication opportunities before adding incremental machinery. Do not move the full-population rebuild from each query to an unconditional high-frequency producer and call it solved. Pose changes, participation hooks, preparation completion, replacement, despawn, and world reset must be reflected together. Freshness checks must continue rejecting stale jump targets and validating commits against current authority; preserving an old immutable snapshot is not proof it is current.

### Phased implementation proposal

1. **Measure the unchanged baseline.** Record pointer sample/submission, host receipt and capture, worker queue wait, entity preparation, candidate simulation, completion polling, and displayed evaluation. Correlate sequences and use each process's monotonic durations without assuming their clocks match. Report median and tail latency, actual candidate/tick counts, body/eligible/local-candidate counts, and allocations or copied-record counts where attributable. Separate publication cost from solver cost. Use a reproducible empty/simple scene, representative populated dungeon, and controlled multipart-door workloads; distinguish synthetic stress from observed density. Run at least five repeats with build profile and workload recorded. Verify browser readiness is `in-world` before invoking precise jump, addressing the previous diagnostic readiness race in the harness. Do not change cadence or search budgets in this phase.
2. **Prototype the immutable obstacle boundary and isolated mover.** Prepare minimal collision records and index once, then reuse them for repeated queries. Adapt the shared contact step to advance one mutable predictor body against immutable targets; remove the need to clone peers every tick and the whole scene per candidate. Reuse the ordinary solver rather than introducing a simplified predictor. Compare trajectories, support/contact outcomes, selected candidate, and rejection reasons with the baseline under identical inputs. Also prove camera-eligible sphere sweep and overlap against the same publication. This phase decides whether the shared boundary is viable and whether the line-count/ownership tradeoff is justified.
3. **Cut precise jump over.** Replace its work-item scene clone and full-body entity snapshot with shared obstacle input plus required mover/authority facts. Preserve latest-aim handling, finite work, cancellation/currentness, entity surface proofs, and fresh commit reevaluation. Ensure cancelled or superseded work cannot accumulate unbounded publications. Delete superseded prediction wrappers and full-body snapshot paths when their consumers migrate; retain no compatibility-only duplicate solver. Re-measure the same workloads and end-to-end latency.
4. **Complete camera entity collision.** Feed client and Explorer boom owners the same query representation. Route initialization, clearance growth, reach sweeps, transit, and overlap recovery through consistent eligible obstacles. Verify closed/open/closing doors, stationary camera recovery, non-door solids, creature exclusion, portal seams, despawn/replacement, and delayed preparation. Preserve existing bounded fallback and coverage semantics. Capture browser evidence that the camera stays on the player's side and the player remains visible.
5. **Evaluate tuning separately.** Only after costs and remaining latency are attributed, compare a higher aim cadence if useful. Treat candidate count, simulation horizon, and evaluation cadence as different controls with different quality/cost effects. A higher search budget requires reachable/unreachable target cases demonstrating useful quality improvement; a higher cadence requires measured responsiveness improvement without unacceptable host/render interference. Keep existing values if the measurements do not justify changing them.

### Acceptance, risks, and scope boundaries

Camera acceptance is the reported obstruction behavior plus recovery, not just a passing sweep unit test. Precise-jump acceptance requires measured improvement on reproducible workloads and no unexplained change in prediction/commit correctness. If copying is minor and simulation or delivery dominates, report that result and address the measured cause rather than forcing a snapshot rewrite as a performance claim. Capture/preparation failures must remain visible and distinguishable from an empty obstacle population.

Largest design risk: the common contact collection currently accepts complete bodies, so reducing prediction to one mover may expose assumptions about peers, support, or directional contact. Resolve those at the smallest shared solver seam and prove equivalence before wiring consumers. Do not turn this into a rewrite of live multi-body simulation, moving-platform physics, collision animation, renderer selection, or worker infrastructure. Snapshot memory retention under slow work and host publication cost are explicit measurements, not presumed negligible.

Use lightweight synthetic regression tests for retained correctness contracts, temporary real-content probes for verification, and browser/live checks for presentation latency and camera behavior. Remove asset-dependent diagnostic tests before commit. Run relevant world/core/host tests, frontend checks where touched, warnings-denied Clippy, formatting, and whitespace checks. No production performance budget or precise sLOC estimate is claimed before baseline/prototype results. The desired reduction is removal of whole-scene/per-tick peer copying and duplicate preparation; new types must each have named runtime consumers.

This joint plan supersedes issue 6's suggestion to adopt the existing full-body entity snapshot directly and the earlier intention to leave precise-jump changes outside scope. Worksheet remains open. The implementation progress below supersedes the original proposal status; tuning remains unchanged.

### Execution progress

The user authorized implementation of the joint plan, stopping for a major blocker/design gap or visual review. The following baseline notes describe the initial investigation; see resumed implementation below for current status. The diagnostic benchmark now separates scene capture and entity snapshot timing and accepts a synthetic peer count. The browser probe now waits for lifecycle `in-world` before the precise-jump shortcut, correcting the previously observed readiness race.

Initial optimized baseline: `00a9011e`, start `(69.098,-67.058,-5.995)`, target `(70,-70,-6)`, 437 center colliders/360 center cells, existing benchmark neighborhood radius 2, six-candidate/160-tick budget. This target selects the first candidate and uses 26 solver ticks. Five runs of 100 evaluations per population give the following median per-evaluation times; brackets are the range of run means, not individual-evaluation percentiles.

| Synthetic peers | Scene clone (µs) | Entity snapshot (µs) | Prediction (µs) |
| --- | --- | --- | --- |
| 0 | 0.075 [0.074–0.078] | 0.045 [0.044–0.046] | 398.597 [388.627–418.885] |
| 16 | 0.800 [0.779–0.928] | 4.781 [4.547–5.021] | 575.180 [562.449–576.178] |
| 64 | 3.576 [3.523–3.912] | 21.084 [20.891–22.038] | 976.936 [973.157–983.787] |
| 256 | 17.853 [17.704–18.983] | 109.808 [108.865–128.165] | 2810.479 [2753.682–2840.162] |

Peers are synthetic cylinder bodies offset away from the short trajectory and seeded with the same cell membership to exercise retained-population preparation. This is deliberately a cost fixture, not an assertion those positions are valid dungeon placements or observed density. It does not validate portal collision, door geometry, or live latency. Initial snapshot construction is a minority of this fixture's cost; per-tick preparation/simulation needs further attribution. Artifacts: `/tmp/shared-query-baseline-repeats.jsonl`, `/tmp/shared-query-baseline-simple.txt`, `/tmp/shared-query-baseline-64.txt`. Debug-build comparison and browser baseline remain in progress. Two sandboxed UI launches exited before DevTools; an approved unsandboxed retry is being checked rather than declaring browser verification unavailable.

Debug comparison completed on the same machine (AMD Ryzen 9 5900X, 24 logical CPUs), five runs of 20 evaluations per population. Median prediction times: 0 peers **3.938 ms** [3.860–4.091]; 16 peers **5.515 ms** [5.367–5.914]; 64 peers **9.025 ms** [8.751–9.367]; 256 peers **23.648 ms** [23.570–24.098]. At 256 peers, median scene capture is 68.575 µs and entity snapshot construction 500.592 µs. Build profile is therefore material; neither these synthetic debug results nor the release measurements identify the user's live latency by themselves. Artifact: `/tmp/shared-query-baseline-debug-repeats.jsonl`.

Execution paused at the requested runtime-verification checkpoint: both unsandboxed Electron launches (X11, then X11 with GPU disabled) retained a defunct Electron child while the CDP operation failed to return. Process inspection proves a terminated Electron child, not which Chromium subprocess role failed or its cause. The helper has unbounded CDP requests, so its outer polling timeout cannot resolve that wait. Both probe-owned process trees were explicitly stopped; no successful world entry, live aim evaluation, or gameplay command was recorded. Artifacts: `/tmp/shared-query-baseline-ui-unsandboxed.jsonl`, `/tmp/shared-query-baseline-ui-software.jsonl` (only launch preambles), and sandbox failure reports `/tmp/shared-query-baseline-ui.jsonl`, `/tmp/shared-query-baseline-ui-x11.jsonl`. Need user confirmation that the ordinary client opens/renders, or evidence of its startup failure, before the required live baseline can proceed reliably. This is the first blocker report, not completion of either issue.

Only diagnostic benchmark, browser readiness guard, and worksheet changed. Benchmark warnings-denied Clippy passes; Rust formatting and whitespace checks pass; the probe script was formatted. No production collision changes, cadence changes, or commit. Remaining work: complete live latency attribution, prototype the shared immutable obstacle boundary, migrate prediction/camera, and verify as scoped above.


### Resumed implementation — shared queries and camera solids

The user confirmed the ordinary client renders correctly. The production cutover is implemented and accepted by the user; the user subsequently requested final code-quality review and commit.

- **World publication:** the canonical body store lazily caches an immutable `Arc<EntityCollisionSnapshot>` containing compact, already-posed collision targets and their index. Entity registration, mutation, replacement, and removal invalidate it. Local-player/predictor mutations do not. Readers retain coherent old publications without mutating peers; failures remain explicit. This describes prepared, retained bodies, not a new guarantee that all server entities have finished preparation. Existing environment-coverage proofs remain environment proofs.
- **Prediction:** work captures the mover and shared obstacles. Each candidate clones only its mover; frozen targets are prepared once per evaluation and reused by the ordinary contact solver on every tick. Removed the full-scene/per-tick-peer cloning path. Frozen-peer behavior, landing proofs, currentness checks, fresh commit evaluation, and work limits remain intact.
- **Camera:** client and Explorer pass environment and entity publications to the shared sphere-query boundary. Reach sweeps, sphere transit, and penetration settlement now include eligible solid obstacles with authored current part poses. The followed body, creatures, ethereal objects, and suppressed/missile participants retain their query-specific exclusions. Explorer captures after entity simulation under the same lock. Capture is demand-driven rather than an unconditional producer rebuild.
- **Regression evidence:** animated solid sweep and overlap, stationary overlap settlement, target/creature/ethereal exclusion, removal, immutable old captures, publication invalidation, and frozen-mover equivalence are covered. A boom regression also verifies an obstruction appearing between an extended camera and its target; existing recovery puts the camera back on the target side without adding another recovery mechanism. These synthetic tests do not substitute for real-door visual acceptance.

Identical optimized workload, five runs of 100 evaluations (median run mean; brackets span run means):

| Synthetic peers | Prediction before (µs) | Prediction after (µs) | Cold publication after (µs, median) |
| --- | --- | --- | --- |
| 0 | 398.597 | 382.111 [373.463–388.100] | 0.048 |
| 16 | 575.180 | 392.200 [388.737–402.737] | 2.563 |
| 64 | 976.936 | 429.523 [424.314–448.493] | 11.934 |
| 256 | 2810.479 | 438.439 [430.795–449.264] | 50.516 |

Warm publication reuse measured below 0.1 µs in this fixture. All 20 optimized runs retain exactly the baseline textual outcome, trajectory, chosen candidate, and 26-tick count. Debug after, five runs of 20: 0 peers **3.678 ms** [3.646–3.942], 16 **3.754 ms** [3.725–3.770], 64 **3.925 ms** [3.862–4.003], 256 **4.312 ms** [4.248–4.336], versus baseline 23.648 ms at 256. Artifacts: `/tmp/shared-query-after-repeats.jsonl`, `/tmp/shared-query-after-debug-repeats.jsonl`. These establish removal of population-dependent repeated work; they do not establish live pointer-to-marker latency or actual dungeon population density. Cadence and candidate/horizon limits remain unchanged.

Validation so far: 1,404 world/core/host library tests passed, plus the subsequently added closing-obstruction regression (1,405 total). Warnings-denied all-target Clippy passes including the final regression; Rust formatting is applied. No asset-dependent permanent tests were added.

The bounded browser probe exposed the earlier launch failure: the X11 override put `--ozone-platform=x11` where Electron expected the app entry argument. Removing that override reached DevTools, then exposed the probe's fixed Vite port colliding with an existing process. The probe now requests the launcher's existing ephemeral-port behavior. Its CDP operations and endpoint fetches have explicit deadlines; precise-jump entry waits for `in-world`. This corrects diagnostic infrastructure, without changing the app's launch policy. A fresh live check is in progress. End-to-end latency attribution and the real closed/open/closing-door visual checks remain outstanding.


Live debug-client smoke check now passes (`/tmp/shared-query-ui-ephemeral.jsonl`): entered world, enabled precise jump, swept 120 pointer inputs over 836 ms, received 26 sweep evaluations, maximum evaluation gap 61 ms, final evaluation 20 ms after input stopped. No page errors or lost WebGL context; probe disconnected cleanly. No movement or jump commit was issued. This is post-change browser evidence, not a before/after latency comparison or per-stage attribution. The earlier live-baseline blocker is resolved; the original full latency measurement phase remains incomplete and no tuning claim rests on this smoke check.

Final Rust formatting, whitespace checks, and frontend ESLint pass.

User acceptance: the user approved the camera fix and then confirmed precise-jump targeting looks noticeably smoother and that all changes look good. Issues 6 and 7 are resolved. Aim cadence remains 30 Hz, with six candidates and 160 simulation ticks per candidate; no resolution or budget increase was needed. Full before/after live latency attribution was not completed and remains a measurement limitation, not a claimed result. Further tuning is deferred unless a new responsiveness issue warrants it. The worksheet remains open for subsequent issues. Final review and commit were subsequently requested.


### Final code-quality review — issues 6 and 7

Reviewed the accumulated diff, including the new camera query module, publication producer and invalidation paths, shared contact-step adaptation, surface-ray consumer, precise-jump submission/worker/currentness/commit path, client camera capture/activation/advance, Explorer collection/start/publication/advance, regression fixtures, benchmark, and CDP helpers. No unresolved blocking design findings remain in this scope.

Fixed one error-boundary finding: Explorer used to propagate camera query preparation failure after committing a simulation collection, discarding its accepted publication. The collection now carries optional camera capture success/failure independently; simulation outputs survive, while the boom holds explicitly on capture failure and recovers on a valid successor. The Explorer adapter regression covers that hold/recovery. Also narrowed the private frozen-step helper, replaced redundant single-result checks with one array conversion, documented private-mover mutation on failure, and removed stale publication/benchmark comments.

Retained tradeoffs: the collision publication rebuilds lazily after any entity mutation rather than maintaining an incremental second registry; frozen prediction peers remain the accepted approximation. Environment coverage does not claim complete entity preparation. No tuning changes, broad live-simulation rewrite, or new frontend contracts. Performance and browser evidence retain the limitations recorded above; user visual acceptance stands.

Final validation: all 1,405 world/core/host library tests passed; host tests passed again with the added capture-failure/recovery assertion. Warnings-denied all-target Clippy for world/core/host/debug-harness, Rust formatting, and whitespace checks pass. Prior frontend ESLint and live aim smoke results remain applicable; no frontend behavior changed during review.

## 8. Player-only entity collision debug toggle

Status: **Resolved; user accepted.** User authorized the scoped implementation on 2026-09-10, accepted the player and boom behavior, and requested quality review and commit. Worksheet remains open.

Client diagnostics now exposes **Disable entity collision**. The core collision coordinator owns a character-scoped override, installs it on only the local player's physical body, retains it across body rebuilds/teleport invalidation, and resets it on leaving the character. Incoming authoritative physics reconfiguration preserves body-local filters. The host command requests the change; a dedicated cold event acknowledges the runtime setting, also included in replacement application snapshots.

The world body filter has an `ENTITY_RESPONSE` exclusion. The existing directional contact decision consumes it for hard-entity sweeps, mobile response, correction clearance, and entity support. Environment collision and topology remain active. Other entities retain their response policies; report eligibility and selection remain independently owned. The client boom now follows the same player entity-response exclusion, as requested in the follow-up below. No peer publication cloning or new collision registry is introduced.

Precise-jump prediction already copies the mover's filter and includes it in authority currentness. Changing the override explicitly invalidates pending/retained core work and cancels the displayed precise-jump mode so an old trajectory cannot remain selectable. Re-entering precise jump uses the accepted filter. Entity support is re-queried during the ordinary next tick, and changing the filter wakes the body for existing support/overlap recovery; no special teleport or ejection policy is added.

Evidence: synthetic regressions cover directional mobile response/restoration, hard contact in both live and frozen prediction with the override on/off, loss of entity support followed by environment landing, and persistence through authoritative reconfiguration/body rebuild followed by logout reset. 1,407 world/core/host library tests pass. The live Electron client was launched with the authorized credentials, the actual debug checkbox clicked on then off, and both runtime acknowledgements and checkbox values matched (`/tmp/entity-toggle-live.jsonl`). No page exception was reported and the probe disconnected cleanly. This live check verifies wiring; the collision behavior evidence is synthetic. All 2,146 frontend tests, Svelte/TypeScript checks, frontend ESLint, and workspace compilation pass. Re-enabling the filter while slightly overlapping a hard obstacle permits retreat using the existing solver; this was checked through the frozen shared contact path. The override is keyed by character GUID, independent of body instance revisions and temporary preparation absence. Warnings-denied Clippy and formatting pass. User subsequently requested quality review and commit.


### Follow-up: apply the player override to the client boom

The user reported that leaving boom entity collision active can strand the camera behind an entity that the player can cross. Client camera capture now reads the followed player's `ENTITY_RESPONSE` filter from the same world epoch. With that exclusion active, its publication omits entity geometry and the existing boom controller receives the environment-only query. Otherwise it receives the ordinary environment-plus-entities query. This covers reach, clearance, transit, and overlap recovery together, including an already active camera; environment collision remains active. Explorer camera policy is unchanged. No independent camera checkbox, shared query redesign, or extra geometry capture is needed.

A synthetic client-camera regression exercises a solid obstacle appearing inside the existing boom, toggles the player filter on, and restores it without restarting the camera. Reach contracts, extends through the entity, and contracts again respectively. The initial fixture pointed the camera into the floor, giving zero baseline reach; correcting the test view direction established the intended unobstructed baseline. This was a fixture issue, not a production change. Validation: all 386 core library tests pass, including the new regression. Warnings-denied all-target Clippy for core/host, Rust formatting, and whitespace checks pass. The live Electron smoke check accepted on/off toggle changes, kept the camera advancing, reported no page exception, and disconnected cleanly (`/tmp/entity-boom-live.jsonl`). The solid-obstacle behavior is covered synthetically; live verification confirms the host/worker toggle path. User accepted the combined player/boom behavior.


### Final code-quality review — issue 8

Reviewed the complete feature diff against its pre-toggle baseline: debug panel and app control, lifecycle transport/validation and replacement snapshots, host command inventory/dispatch/projection/protocol, core command handling and teardown, collision coordinator preparation/reconfiguration, body exclusion mutation, directional contact/support/report consumers, frozen-prediction authority and cancellation, camera capture/query selection, and regression fixtures. The existing typed host plumbing is retained; no parallel settings service or collision registry was introduced. The worksheet remains open for the next issue.

Fixed a preparation-race defect: the command previously rejected an absent physical body, which the shared command loop treats as a terminal runtime failure. The coordinator now accepts character-scoped policy while that body is preparing and installs it with the body; a request overtaken by character teardown acknowledges the retained state. Added coverage for acceptance before initial physical preparation. Narrowed the world mutation to selected exclusions, preserving independently owned domains when toggling or resetting the debug override, and extended the coordinator regression to verify that preservation. Also restored an unrelated test's original executor configuration, corrected a misplaced decoder comment, moved the session-unavailable error inside the UI error handler, narrowed coordinator method visibility, and reused camera settlement timing constants in its regression.

No unresolved blocking findings remain. Environment/report/selection behavior retains the scoped independence described above; the camera intentionally shares the player's entity exclusion. Prior live toggle/boom smoke checks remain applicable to the unchanged normal command path. The preparation-gap and independent-exclusion changes were verified synthetically. Final validation: 1,408 world/core/host library tests (739/386/283) and all 2,146 frontend tests pass. Core was rerun after the final regression/visibility cleanup. Warnings-denied all-target Clippy, Svelte/TypeScript checks, frontend ESLint, formatting, and whitespace checks pass.


## Issue 9 — airborne trajectory stutter

Status: resolved; user accepted the core camera timing correction. The separately proven frontend playback defect is deferred as a follow-up, not claimed fixed.

### Initial evidence

The frontend restarts each integrated entity path on local receipt. `ClientPresentationSession` stamps `performance.now()` on the dynamic tick, `GamePresentationRuntime.applyDynamicEntityTick` forwards that receipt, and `DynamicEntityPlacementSystem.applyPath` replaces any existing path and immediately presents the new initial point. `advance` clamps at the endpoint and retires expired paths. Consequently early delivery skips unfinished travel, and late delivery leaves a stationary gap. This is independent of server correction and does not require bad collision geometry.

A temporary diagnostic exercised the production placement system with contiguous 30 ms paths at 10 m/s. Receipts at 1000, 1040, and 1060 ms produce a 10 ms stationary gap, then a jump from 0.5 to 0.6 m at the same presentation time when the third segment arrives. All five tests in the diagnostic file passed (four existing placement tests plus the probe). The probe is preserved at `/tmp/airborne-delivery-probe.test.ts` and removed from the source tree. Timing and speed are controlled experimental inputs, not measured live cadence or player speed.

The camera has a different active/pending playback policy (`ClientCameraSession`), and the core camera independently consumes each newly observed player travel once, otherwise sampling the current pose (`ClientCameraController.advance`). Player/camera phase disagreement is therefore another boundary to measure. This code evidence does not yet prove the observed airborne symptom is wholly caused by presentation delivery, or exclude physical/server corrections.

Next diagnostic: correlate accepted player path endpoints, discontinuous corrections, frontend receipts and frame-sampled player/camera placements during a jump and fall. Distinguish world-space trajectory discontinuities from camera-relative motion. Candidate correction, pending attribution: preserve a continuous bounded presentation timeline and coordinate player/camera sample time, retaining authoritative portal path legs and explicit resets. Account for buffering latency and late-sample policy; do not hide this with arbitrary positional smoothing or change gravity/network cadence without evidence.


### Problem-solving pass — timing ownership

**Constraints.** Preserve collision-approved path legs, exact portal membership and camera clearance. Never interpolate a straight shortcut between endpoints across obstacles. Teleports, correction snaps, character generations, projection changes and activation remain explicit discontinuities. Keep camera orbit responsive independently of physics publication. Keep queues and latency bounded; presentation must not alter physics, outbound movement or server reconciliation. Sample player and camera against compatible source times. A local receipt timestamp is not a simulation timestamp.

**Distribution.** Physics is nominally 30 ms (`client/mod.rs`, runtime interval uses missed-tick Skip); the camera worker targets 16 ms and passes actual elapsed duration (`camera_service.rs`). Browser rendering and transport delivery are independent. Dynamic batches carry host publication time and actual simulation duration; paths can contain portal/contact legs, integrated travel or explicit resets. A host publication timestamp is measured at publication, so its equivalence to the simulated interval endpoint must be established rather than assumed. Camera outputs carry sequence and worker duration, but no corresponding shared source-time interval.

**Additional structural evidence.** `CameraBodyInput.travel` retains only batch host time and geometry: capture discards `batch.duration_ms`. `ClientCameraController.advance` consumes all normalized path legs once in a worker advance, then samples the endpoint until new travel appears. Thus a nominal 30 ms player path is handed to a roughly 16 ms camera solve without its original duration. This can generate uneven target progression even with perfectly regular frontend receipts. This is code-proven loss of timing information; its visual contribution in the reported live scene still needs measurement.

**Neighborhood and ownership.** Core simulation owns accepted motion and its temporal meaning. Core camera orchestration owns consuming that motion over time and collision-solving camera movement; the world boom remains a geometric/controller consumer of correctly timed target samples. The frontend owns bounded playback and mapping source time to render time. Existing placed-path evaluators retain ownership of interpolation and membership. Receipt scheduling must not independently redefine movement speed in either entity or camera playback.

**Concessions.** After a real stall, holding the last proven placement and explicitly resynchronizing is acceptable; unlimited catch-up backlog and extrapolation through unverified geometry are not. A small bounded presentation delay may be required to absorb ordinary jitter, but its size must be measured and weighed against input latency. Do not select a buffer size from nominal tick frequency alone.

**Candidate direction.** First preserve explicit movement interval semantics at the camera boundary and consume only the time slice due, retaining unconsumed accepted travel. Then coordinate entity/camera presentation against source-time intervals with a bounded playout policy, replacing independently receipt-started playback. Do not simply copy the camera's existing two-path queue to entities: it still restarts after underflow and drops/restarts after overflow, and cannot repair the upstream lost duration. A shared clock does not by itself make camera paths describe the same player time; target timing must be corrected first. Avoid a general animation/scheduling framework or changes to gravity, server update cadence, and collision solvers.

**Verification before implementation scope is final.** Reproduce camera target progression with nominal physics and worker cadences and varying phase; record path input duration versus consumed fractions and target positions. Then capture a live jump/fall with accepted physical motion, correction events, camera target/output and frame samples on correlated clocks. Compare stationary-camera and following-camera views where practical. Regression matrix: regular/jittered/batched delivery, underflow/overflow, apex/landing, portal crossing, teleport/reset, camera orbit without physics updates, and projection clearance changes. The prior frontend probe proves a failure mechanism, not complete attribution. No implementation authorized or performed by this pass.


### Staged scope — issue 9

Goal: remove the reported airborne stutter with the smallest timing correction supported by measured evidence. This scope supersedes treating coordinated frontend playback as an unconditional implementation step. Phases 1 and 2 are complete and user-accepted; the checkpoint selected deferring the broader frontend change.

Guiding principles: preserve temporal meaning alongside accepted geometry; keep camera input independently responsive; reduce competing playback policies where warranted; prove the reported symptom improves before expanding the design.

#### Phase 1 — attribute the symptom

- [x] Add temporary diagnostics or focused harness instrumentation around accepted player paths, correction kinds, camera target sampling/output, receipt times and rendered player/camera placement. Use existing live UI probe infrastructure; never print credentials.
- [x] Exercise a jump and a fall, capturing both world-space player travel and camera-relative displacement. Record actual durations, publication/receipt gaps and frame intervals. Host and browser clocks need an explicit correlation; do not subtract their raw timestamps.
- [x] Reproduce camera target movement under regular nominal 30 ms physics and 16 ms camera service with several relative phases. Compare source path fractions with fractions consumed by the camera. Include delayed and overwritten publications. Nominal periods are test inputs; production uses actual elapsed duration.
- [x] Separate accepted-physics discontinuities, camera target timing distortion and frontend delivery distortion. If accepted physics/corrections account for the observed symptom, redirect investigation before implementing a playback fix.

Touch points: `crates/holtburger-core/src/client/{runtime.rs,dynamic_entity_view.rs,camera.rs,camera_service.rs}`, `apps/holtburger-3d/src/client/client-presentation-session.ts`, `apps/holtburger-3d/src/lib/game/{camera/client-camera-session.ts,systems/dynamic-entity-placement-system.ts}`, and `apps/holtburger-3d/scripts/live-client-ui-probe.mjs` or temporary diagnostic equivalents.

Exit criterion: a reproducible camera timing trace plus an attributed live capture, or an explicitly documented remaining attribution gap. Synthetic evidence alone does not establish that the live symptom is fixed.

#### Phase 2 — bounded core camera prototype

Proceed if phase 1 supports the camera timing mechanism. Primary implementation boundary is core camera orchestration; retain the current physics cadence, world collision solver and frontend wire shapes for this experiment unless measurement proves that boundary insufficient.

- [x] Retain accepted path duration with its publication identity in one composite travel value. Establish whether a relative playback cursor suffices for the prototype; do not treat publication time as the simulation endpoint without tracing its producer.
- [x] Consume only the remaining travel appropriate to elapsed camera service time. Carry any unfinished portion forward; handle a service interval spanning path boundaries. Preserve the exact ordered portal/contact legs and placement at slice boundaries using existing path/child-body operations where possible.
- [x] Account explicitly for latest-only `CameraState.input`: an active path can be retained locally, but intermediate publications can disappear before the worker sees them. Define a bounded pending-path policy if required and an explicit reseed/hold path for missing continuity. Do not splice across unobserved portal travel or quietly discard unfinished motion.
- [x] Keep current collision queries coherent with each solve, preserve entity-collision exclusion, and keep orbit/zoom and projection clearance independently serviceable while target travel is exhausted or held. Delayed target motion must still be validated against the available scene; retain existing recovery behavior when coverage is unavailable.
- [x] Retire retained travel on camera restart, player generation change, activation and discontinuous correction. Confirm non-integrated publications cannot replay stale movement.
- [x] Compare regular-phase traces and the same live jump/fall with baseline. Measure camera/player relative phase and added follow latency; spreading camera movement across the correct duration can expose a frontend phase mismatch rather than cure it.

Expected footprint: a focused change in `camera.rs`, possibly the publication handoff in `camera_service.rs`, and colocated tests. A general timeline service, broad movement-contract migration, or queue of full collision snapshots exceeds this prototype's scope. No line-count estimate is defensible until slice/reconciliation reuse is confirmed; duplicated path traversal or substantial new state triggers design reassessment.

Exit criterion: source movement duration is preserved across camera service phases, bounded recovery is exercised, existing placement/clearance contracts pass, and a live before/after result is recorded. A successful synthetic prototype alone is insufficient to call the issue resolved.

#### Checkpoint — stop at the accepted core correction

- [x] User accepted phase 2; retain its regressions and documented verification.
- [x] Record the independently proven frontend delivery defect as a deferred follow-up. Do not expand this issue into coordinated frontend playback.

#### Deferred follow-up — coordinated frontend presentation (formerly conditional phase 3)

Deferred after user acceptance of phase 2; not required to close issue 9. If a subsequent report warrants this work, scope it separately. Define source interval semantics at their producer and carry the required temporal relationship through camera output. Then give frontend entity/camera playback one compatible time mapping and bounded buffering policy, reusing existing placed-path evaluators. Quantify steady-state delay, jitter tolerance and stall resynchronization before selecting a buffer. Consider remote entities and Explorer consumers explicitly before changing shared contracts. Keep animation, selection and camera clearance aligned with the placement actually presented. This is a candidate direction, not a preapproved general scheduler implementation.

#### Final phase — cleanup and verification

- [x] Retain small synthetic regressions for the demonstrated failure and the selected timing policy; remove temporary instrumentation and asset-dependent tests. Remove superseded consumed-once state if the prototype replaces it; do not retain parallel production modes.
- [x] Check regular/jittered service, missed publication, path exhaustion, apex/landing, portal crossing, teleport/reset, stationary target with orbit input, projection changes and entity-collision toggle. Scale tests to the boundaries actually changed.
- [x] Run affected Rust tests, formatting and warnings-denied Clippy; run frontend tests/type checks when frontend contracts or playback change. Perform live browser verification for any shipped behavior change.
- [x] Record evidence, limitations, latency impact and user acceptance here. Keep the worksheet open for subsequent issues. Commit only when requested.

Definition of done: user-observed airborne stutter resolved; accepted physics and portal/camera collision behavior preserved; no accumulating latency or unbounded queues; relevant checks and live verification pass; diagnostic debt removed. The broader frontend defect may remain explicitly deferred only if it is no longer required to resolve this issue.

Remaining measurement limitation: exact end-to-end phase and added latency remain unmeasured. Phase 2 established path slicing through the existing child-body solver and bounded retention at the core handoff; user acceptance closes the reported symptom. Further frontend timing design belongs to the deferred follow-up.


### Phase 1 results — outdoor live attribution

Phase 1 completed as a diagnostic-only step, with no production changes retained at that checkpoint. Phase 2 results follow below. The user moved the character outside before the second capture and authorized retrying there.

**Capture.** Live Electron client, debug development build, current character at outdoor landblock `0x7d64ffff` (no EnvCell), three stationary jumps with an 800 ms charge and 4 s settling between releases. Each arc rose approximately 16.18–16.20 m and lasted approximately 3.75–3.78 s from the first airborne publication through landing. Ascending and descending portions are captured; there was no separate walk-off-ledge fall. The earlier indoor capture was preserved but is not used for the outdoor measurements. No camera gesture was applied. Probe disconnected cleanly and reported no page exception.

**Physical motion.** Outdoor trace contains 380 local-player advances: all `integrated`, 377 labelled airborne and three grounded. Consecutive accepted vertical path endpoints/starts match exactly; there are no correction snaps, teleports or resets during capture. Ordinary interior acceleration is near gravity; landing/contact intervals naturally differ and must not be treated as ballistic samples. Actual simulation durations: median 30.071 ms, p95 30.524 ms, range 29.074–30.935 ms. The evidence does not implicate server correction or collision seams in this reproduction.

**Core camera attribution.** Camera service duration median 16.067 ms, p95 16.076 ms, range 16.014–16.137 ms. All sampled in-flight camera outputs are `advanced`. In a roughly 170 ms ascent excerpt, player path average vertical speed decreases smoothly through 12.678, 12.385, 12.090, 11.794, 11.500 and 11.205 m/s. Camera output pivot speed in the same browser receipt window reads 2.427, 20.629, 23.036, 2.426, 20.235, 2.426, 19.755, 2.427, 19.063, 2.427 and 18.528 m/s. These camera speeds use the host-authored path displacement divided by its own worker duration, so browser timing jitter cannot create this oscillation. The rendered camera also exhibits the uneven progression.

The plain move/hold description from initial inspection needs refinement: the raw target advances to the newly published endpoint, but the boom already applies vertical smoothing. `KinematicBoomController::filter_pivot` in `crates/holtburger-core/src/kinematic_boom.rs` uses an 80 ms half-life and a 0.30 m maximum vertical lag. At fast airborne speeds, new raw target steps engage the lag clamp; intervening updates smooth toward an unchanged target. The approximately 2.427 m/s intervals are consistent with smoothing a 0.30 m lag over a 16 ms update. The current smoothing therefore does not remove the wrong target-time progression. This supports fixing timing before considering tuning the filter.

**Independent synthetic reproduction.** A temporary core test exercised production camera capture/controller against the checked-in synthetic floor fixture, supplying contiguous 0.15 m upward movement increments at nominal 30 ms physics cadence and servicing at 16 ms. Relative phases 0, 7 and 15 ms all exhibit uneven output; after the initial smoothing transient, typical successive displacements are 0.111165 m and 0.038835 m per 16 ms camera service. This is a controlled 5 m/s source, not the live player's speed. A 96 ms worker gap advances the latest observed publication identity from 2 to 5; publications 3 and 4 are unavailable in the latest-only handoff. The test passes and records the behavior; it is diagnostic evidence, not a proposed regression expectation. Portal safety during missing travel remains phase 2 design/verification work.

**Frontend contribution.** Applying the production replacement/clamp policy to consecutive airborne receipt samples yields an early-replacement vertical skip p95 of 2.87 cm, maximum 7.62 cm; late-path holds p95 3.454 ms, maximum 5.977 ms. These are reconstructed policy discontinuities at receipt instants, not assertions that every maximum was visibly sampled by a frame. Actual rendered player/camera placements were captured after rendering in the same frame hook. Browser frame intervals were median 6.9 ms, p95 7.0 ms, maximum 20.9 ms. There was no broad frame-rate collapse explaining the periodic camera oscillation.

**Clock discipline.** Physics and camera velocities are computed using each output's own authored duration, not receipt gaps. Receipt-induced replacement/hold calculations and frame samples use the same browser `performance` clock. For matching excerpt windows, dynamic publication time plus its observed minimum receipt offset provides a relative alignment with unknown baseline transport delay; observed receipt-minus-host offset spans 411.473–417.650 ms and must not be reported as latency. Camera outputs lack a shared host timestamp, so exact end-to-end phase/latency is still unmeasured. No cross-process raw clock subtraction is used as a latency claim. The pre-delivery camera speed oscillation is established independently of that remaining gap.

**Decision.** Proceed to the scoped phase 2 camera timing prototype when authorized. The source-time distortion is a demonstrated contributor in the user's live outdoor scene. Do not tune gravity, movement network cadence, frame rate or the vertical filter to mask it. Retain the frontend defect as a proven second contribution and measure residual stutter after the core prototype. Phase 1 does not establish that a camera-only fix will fully resolve the report, nor prove the same attribution for a separate ledge fall.

Artifacts: `/tmp/airborne-live-data.json` (outdoor raw events and presented frames), `/tmp/airborne-live-log.jsonl`, `/tmp/airborne-indoor-data.json`, `/tmp/airborne-player-paths.json`, `/tmp/analyze-airborne.py`, `/tmp/airborne-cadence.txt`, `/tmp/airborne-camera-diagnostic.rs`, `/tmp/airborne-session-instrumented.ts`, `/tmp/airborne-live-probe.mjs`. Temporary source instrumentation and the synthetic diagnostic test have been removed; the only remaining tracked diff is this worksheet. No commit. The initial sandbox launch failed with loopback `EPERM`; approved live launches succeeded.


### Phase 2 results — timed core camera following

Implemented on user authorization and accepted after visual testing; final code-quality review and commit were subsequently requested. The worksheet stays open. The browser player/camera playback contracts and smoothing profile are unchanged.

**Final prototype shape.** `camera/target_playback.rs` owns an actual-duration cursor over immutable accepted parent paths. Camera capture retains either timed travel or an explicit correction/reset publication. The publication handoff in `camera_service.rs` delivers each update to this owner before replacing the latest scene input; it therefore does not depend on the worker observing every publication. Each service consumes the due portion, preserves crossed authored corners and zero-time corrections, and carries unfinished travel forward. Partial poses are coordinate-only parent points; the existing child-body solver derives the camera target's portal placement against the current collision scene. No old scene snapshots or entity bodies are retained in the playback queue.

History is bounded to two source paths (current interval and successor) and at most four nominal physics intervals of remaining travel (currently 120 ms). The time cap handles exceptionally long source intervals; it is a recovery ceiling, not an added steady-state buffer. These bounds are local prototype recovery policy, not a measured optimum. Missing path continuity, overflow, explicit correction/teleport/reset, or target/controller failure retires history and triggers safe target placement initialization. `KinematicBoomController::invalidate_target_path` preserves operator intent and requested projection clearance while requiring the next solve to prove placement at its target instead of traversing an invented route. Initial registration starts at the current accepted endpoint and does not rewind pre-registration travel. Ordinary orbit/zoom and entity-collision exclusion continue through the same controller/query path.

**Regression evidence.** Tests cover partial consumption across publications, exhausted travel without replay, ordered corners/zero-time corrections, missing publication continuity, queue overflow, unchanged-endpoint resets and long-interval recovery. A synthetic indoor-to-outdoor portal test verifies that slicing does not prematurely commit the child outdoors. A full camera-controller test with a controlled 5 m/s source and 30/16 ms clocks at phases 0, 7 and 15 ms verifies that settled camera output remains within 1 m/s of the source speed, instead of alternating between near-stationary and excessive-speed output. These clocks and tolerances are explicit experimental inputs, not runtime tuning assumptions. Existing collision-toggle, orbit, registration, coverage, projection and lifecycle checks remain applicable.

**Live comparison.** Repeated the phase 1 outdoor workload: same character, `0x7d64ffff`, three stationary 800 ms-charge jumps, unchanged debug client and frontend policy. Each arc rose approximately 16.18–16.23 m. All 380 player advances remained integrated and vertical endpoints remained continuous; no correction snaps occurred. All sampled interior-airborne camera ticks were advanced. The probe disconnected cleanly without a page exception. Temporary frontend frame instrumentation was removed afterwards.

For each arc, take the absolute difference in vertical pivot speed between successive host camera outputs, excluding the first/last 100 ms of airborne capture. This measures output unevenness before browser delivery, not CPU performance:

| Arc | Baseline median / p95 speed change (m/s) | Prototype median / p95 (m/s) |
| --- | --- | --- |
| 1 | 10.454 / 25.205 | 0.150 / 0.286 |
| 2 | 10.157 / 25.592 | 0.154 / 0.337 |
| 3 | 10.298 / 26.072 | 0.150 / 0.291 |

The prototype removes the repeated large core-camera oscillation in these captures. Isolated maxima remain (up to 10.418 m/s speed change in arc 1), so this is not a claim of perfectly uniform output. A smooth gravity-driven arc naturally changes speed by approximately 0.157 m/s per 16 ms interval. Remaining rendered player and camera spikes include frontend delivery behavior; one source delivery gap was roughly 20 ms above the minimum observed host-to-browser offset, and the current receipt-clocked playback still skips/holds under those conditions.

**Latency limitation.** Browser-receipt timing of the source apex versus camera-pivot apex was 103.4/110.9/114.2 ms before and 125.7/117.1/119.1 ms after. The median shift is 8.2 ms, smaller than source sampling granularity; this is only a coarse relative follow-lag check, not an exact added-latency measurement. The existing 80 ms-half-life vertical filter remains active. Host camera events still lack a shared source timestamp, so exact end-to-end phase attribution remains outside this narrow prototype.

**Scope checkpoint — accepted.** The user reported, “yeah I think this is acceptable now,” and approved treating the airborne stutter as resolved with the core camera correction. Conditional phase 3 is deferred as a separate frontend playback follow-up. The receipt/playback defect and remaining measured spikes are not claimed fixed; they do not require expanding this accepted issue. No gravity, server position-update, renderer cadence or vertical-filter tuning changes were made. The worksheet remains open for subsequent issues; the user subsequently requested a code-quality pass and commit.

Artifacts: baseline `/tmp/airborne-baseline-outdoor.json`, after `/tmp/airborne-live-data.json`, comparison `/tmp/compare-airborne.py`, source-speed regression `/tmp/airborne-speed-test.txt`, portal regression `/tmp/airborne-portal-test.txt`, full test logs `/tmp/airborne-phase2-all-tests.txt` and `/tmp/airborne-phase2-final-core.txt`, Clippy `/tmp/airborne-phase2-clippy.txt`. Final validation: 392 core and 283 host library tests pass (675 total), including the added portal test; warnings-denied all-target core/host Clippy, Rust formatting and whitespace checks pass. No retained frontend change, protocol shape change or temporary diagnostic test.


### Final code-quality review — issue 9

Reviewed the complete issue diff against the current branch baseline, including the new target-playback module. Seam coverage: actual-duration dynamic batch production and camera capture; publication under the camera-service lifecycle lock; duplicate observation, bounded retention and partial consumption; child-body portal reconstruction; boom clearance and failure/reseed paths; output projection; host event forwarding; and the unchanged frontend camera playback consumer. Traced initial registration, explicit correction, exhausted travel, missing history, overflow, absent target and held/fallback output. The bounded cursor remains in core camera orchestration; no scene-snapshot queue, renderer timing service or duplicate collision solver was added.

Removed an unreachable interpolation fallback: after exact-boundary handling, the ordered-waypoint contract guarantees a positive span for an interior sample. Added a regression verifying that target-route invalidation reproves placement at the destination while retaining orbit intent, desired zoom reach and pending projection clearance. The first assertion compared two equivalent outdoor selector representations; it was corrected to compare world-space placement and committed cell. This was a test comparison issue, not a production failure. Existing live before/after and user acceptance remain applicable because the review cleanup does not change valid ordinary playback.

No unresolved blocking findings remain within this feature's scope. The separately documented frontend receipt-clock defect stays deferred, and this change does not redesign the generic boom's handling of instantaneous corrections. Final verification: 393 core and 283 host library tests pass (676 total); warnings-denied all-target core/host Clippy, Rust formatting and whitespace checks pass. Temporary diagnostics remain outside the worktree. Only issue 9 implementation/tests and this worksheet are included in the requested commit; unrelated ACE/ACViewer changes remain untouched.
