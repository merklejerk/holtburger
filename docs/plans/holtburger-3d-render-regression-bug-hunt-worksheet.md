...
- `npm run --prefix apps/holtburger-3d test:ts -- src/lib/assets/prepared-asset-store.test.ts src/lib/assets/material-diagnostics.test.ts src/lib/world-display/browser-render-resource-coordinator.test.ts src/lib/world-display/terrain-materials.test.ts src/lib/world-display/model.test.ts`
- `npm run --prefix apps/holtburger-3d test:ts`
- `npm run --prefix apps/holtburger-3d lint:ts`
- `npm run --prefix apps/holtburger-3d check`
- `rg "describeAssetStateSignature|recordPreparedAssetSignatureDiagnostics|asset-state signature|preparedByAssetId.*sort\\(\\).*join" apps/holtburger-3d/src` returned no matches.
- `rg "assetState\\.preparedByAssetId|assetState\\.cacheMetadataByAssetId|frontendState\\..*preparedByAssetId|frontendState\\..*cacheMetadataByAssetId" apps/holtburger-3d/src/app apps/holtburger-3d/src/pages/BrowserWorldDisplay.svelte apps/holtburger-3d/src/lib/world-display/browser-render-resource-coordinator.ts` returned no matches.
...
- Transitional record-shaped prune snapshot debt is promoted to Phase 21G so Phase 22 does not bake legacy cache snapshots into the new runtime boundary.
...
- Remove the remaining record-shaped prepared-cache dependencies from the asset runtime before lifting broader scene-resource ownership in Phase 22.

Context:

- Phase 21B moved authoritative prepared asset ownership into `PreparedAssetStore`, and Phase 21D made the renderer subscribe to `PreparedAssetResolver` directly.
- Phase 21F made pruning independently scheduled and bounded, but `SceneAssetStreamingController` still asks its caller for `Record<string, PreparedAssetRecord>` / cache metadata snapshots.
- `App.svelte` still builds full legacy snapshots with `createPreparedAssetLegacySnapshotFromResolver(...)` for scene interest sync, request planning, and prune planning.
- If Phase 22 creates `ClientAssetRuntime` / `SceneResourceRuntime` on top of these snapshot APIs, the new runtime boundary will inherit the old cache shape. That would be clean-looking technical debt, which is worse because it wears a suit.

Deliverables:

- Extend `PreparedAssetResolver` / `PreparedAssetStore` with explicit read APIs needed by request planning and prune planning:
  - direct prepared asset lookup,
  - cache metadata lookup,
  - bounded iteration or cursor-based scan for prune candidates,
  - count/diagnostic access only where it is cadence/on-demand safe.
- Refactor `SceneAssetStreamingController` dependencies away from `getPreparedByAssetId()` and `getCacheMetadataByAssetId()` record snapshots.
- Refactor scene asset request planning and static bundle layer planning call sites touched by the controller to use resolver/read-model inputs where practical. If a planner still needs a snapshot temporarily, name it as an explicit off-frame planning snapshot and register it in Phase 23.
- Refactor prune planning so the duty cycle advances over store/resolver-owned entries without constructing a full prepared/cache record snapshot per tick.
- Remove `App.svelte` calls to `createPreparedAssetLegacySnapshotFromResolver(...)` for runtime sync/prune paths.
- Keep `frontendState.applyPreparedAssets(...)` only for compact presentation/error/pending state. Do not restore full prepared cache ownership to Svelte.

Acceptance criteria:

- `SceneAssetStreamingController` no longer has dependency fields named `getPreparedByAssetId` or `getCacheMetadataByAssetId`.
- `App.svelte` does not call `createPreparedAssetLegacySnapshotFromResolver(...)` for runtime scene sync or pruning.
- Prune ticks do not call `Object.keys(preparedByAssetId)` or construct full cache snapshots to choose a bounded eviction batch.
- Request planning still avoids duplicate requests for already-prepared or pending assets.
- Asset/runtime tests prove resolver-native request planning and bounded prune iteration.
- Any remaining record-shaped prepared asset snapshot helper is either outside runtime hot paths or tracked in Phase 23 with an owner and deletion condition.

Verification:

- Add or update focused tests for `PreparedAssetStore` / `PreparedAssetResolver` native lookup and bounded iteration.
- Add or update `SceneAssetStreamingController` tests proving request sync and prune ticks work without record snapshot dependencies.
- Run:
  - `npm run --prefix apps/holtburger-3d test:ts -- src/lib/assets/prepared-asset-store.test.ts src/lib/assets/scene-asset-streaming-controller.test.ts src/lib/assets/asset-cache-policy.test.ts`
  - `npm run --prefix apps/holtburger-3d test:ts`
  - `npm run --prefix apps/holtburger-3d lint:ts`
  - `npm run --prefix apps/holtburger-3d check`
- Grep verification:
  - `rg "getPreparedByAssetId|getCacheMetadataByAssetId|createPreparedAssetLegacySnapshotFromResolver" apps/holtburger-3d/src/App.svelte apps/holtburger-3d/src/lib/assets`

Implementation notes:

- Added `PreparedAssetResolver.scanPreparedAssets(...)` and store/native test coverage so prune planning can read bounded candidate pages with cache metadata without constructing full prepared/cache record snapshots.
- Added `planPreparedAssetCachePruneBatchFromResolver(...)` for runtime pruning. The old record-shaped cache policy remains for existing planner/tests, but `SceneAssetStreamingController` no longer calls it.
- Replaced `SceneAssetStreamingController` snapshot deps with a required `preparedAssetResolver`; scene interest no longer carries `preparedByAssetId`.
- Replaced the old prepared-asset ID signature in the streamer sync key with `PreparedAssetResolver.getPreparedRevision()`. The controller can run another planning pass after resolver updates without forcing Svelte to build or pass cache snapshots.
- `App.svelte` no longer imports or calls `createPreparedAssetLegacySnapshotFromResolver(...)`; its frontend-state subscription now only invokes runtime scene sync when browser scene-interest fields change.
- Request planning still consumes a record-shaped snapshot through explicitly named `createOffFrameRequestPlanningSnapshot(...)` inside `SceneAssetStreamingController`. This is tracked in Phase 23 as off-frame planner compatibility debt rather than a runtime sync/prune dependency.

Verification results:

- `npm run --prefix apps/holtburger-3d test:ts -- src/lib/assets/prepared-asset-store.test.ts src/lib/assets/scene-asset-streaming-controller.test.ts src/lib/assets/asset-cache-policy.test.ts` passed: 3 files, 12 tests.
- `npm run --prefix apps/holtburger-3d test:ts` passed: 95 files, 522 tests.
- `npm run --prefix apps/holtburger-3d lint:ts` passed.
- `npm run --prefix apps/holtburger-3d check` passed with 0 errors and 0 warnings.
- Grep result for `getPreparedByAssetId|getCacheMetadataByAssetId|createPreparedAssetLegacySnapshotFromResolver` no longer shows `App.svelte` or runtime sync/prune usage. Remaining matches are the legacy helper itself and `material-diagnostics.ts`, which is diagnostic/presentation compatibility debt tracked in Phase 23.
...
- Phase 21 created the prepared asset resolver/event seam and removed the public renderer `setAssetState()` bridge. Phase 22 should use the same event/source pattern for landblock products rather than reintroducing browser-display middleman behavior.
- Phase 21G should remove the asset-runtime record-shaped snapshot dependency before Phase 22 starts. Phase 22 may still encounter lower-level renderer compatibility snapshots, but it must not introduce new runtime services that depend on Svelte-owned or full-record prepared cache mirrors.
...
- Migrating every lower-level renderer helper off `AssetChannelState`; that is Phase 23 cleanup unless a helper is directly needed for the product-source cutover.
...
- Add a renderer-facing `StaticLandblockProductSource` / `LandblockProductSource` abstraction with:
  - current product-set read access for initial hydration,
  - bounded commit/evict/clear events,
  - subscription cleanup,
  - no Svelte dependency.
- Extend or wrap `StaticLandblockRenderArtifactStore` so product changes are observable before ownership moves out of `BrowserWorldDisplay.svelte`.
- Keep product events payload-shaped, not whole-app-state-shaped. Product commits may carry a single `LandblockRenderProductWorkerResult`; evictions carry `StaticLandblockProductKey`; clears are explicit.
- Add focused product-source tests before moving coordinator ownership.
...
- Define a mode-neutral scene-interest DTO consumed by asset and product runtimes. Browser mode can populate it from destination plus LoD radii today; normal client mode can populate it from future client/server world state later.
- Define runtime/service boundaries with client-neutral names. Candidate split:
  - `ClientAssetRuntime` for asset channel, prepared asset store, streaming, retention, and diagnostics snapshots.
  - `LandblockProductRuntime` for desired product planning, worker client, product source/store, in-flight state, product diagnostics, and product events.
...
- Prefer a small app-local composition module over making `App.svelte` a behavior-heavy orchestration object.
...
- Require `WorldDisplay` / renderer construction to receive:
  ...
  - landblock product source/event stream
  - any other renderer-required runtime source that is not browser presentation state
- Make `WorldDisplay` subscribe once to the product source at construction, seed initial static products from the source, and unsubscribe on dispose.
- Queue product source events and flush them at renderer-controlled sync points rather than mutating expensive GPU resources from browser-display callbacks.
- Delete or deprecate the `WorldDisplay`/renderer imperative product-forwarding surface (`commitStaticLandblockProduct`, `evictStaticLandblockProduct`, `clearStaticLandblockProducts`) once product-source delivery is live.
...
- Product events reach renderer/resource dirty queues without `BrowserWorldDisplay.svelte` forwarding methods.
...
- `rg "commitStaticLandblockProduct|evictStaticLandblockProduct|clearStaticLandblockProducts" apps/holtburger-3d/src/pages apps/holtburger-3d/src/lib/world-display` shows no browser-display forwarding path after the cutover, aside from intentional product-source internals/tests.

Progress:

- Made `WorldDisplay` require a `StaticLandblockProductSource` at construction, seed its initial product set from the source, and subscribe once to product source events.
- Routed product source events through the renderer as `applyStaticLandblockProductEvent(...)`, preserving current WebGL commit/evict/clear resource behavior behind an event-shaped renderer sink.
- Removed the `BrowserWorldDisplay.svelte` product forwarding callbacks and passed `staticLandblockRenderCoordinator.productSource` into `WorldDisplay`.
- Removed product methods from `BrowserRenderResourceSurface`; the browser render-resource coordinator no longer has a static-product commit surface.

Verification:

- `npm run --prefix apps/holtburger-3d test:ts -- src/lib/world-display/static-landblock-product-source.test.ts src/lib/world-display/static-landblock-render-artifact-coordinator.test.ts src/lib/world-display/browser-render-resource-coordinator.test.ts` passed: 3 files, 13 tests.
- `npm run --prefix apps/holtburger-3d test:ts` passed: 98 files, 526 tests.
- `npm run --prefix apps/holtburger-3d lint:ts` passed.
- `npm run --prefix apps/holtburger-3d check` passed with 0 errors and 0 warnings.
- `rg "commitStaticLandblockProduct|evictStaticLandblockProduct|clearStaticLandblockProducts" apps/holtburger-3d/src/pages apps/holtburger-3d/src/lib/world-display` returned no matches.

### Phase 22D: Move Landblock Product Runtime Ownership Out Of Browser Display

Status: complete.

Deliverables:

- Move `StaticLandblockRenderArtifactCoordinator` ownership out of `BrowserWorldDisplay.svelte` into `LandblockProductRuntime` or the `SceneResourceRuntime` composition.
- Let the landblock product runtime own the coordinator, worker client, product source/store, in-flight state, product diagnostics, and product events.
- Feed runtime scene-interest updates from browser-mode code without letting browser presentation state enter the runtime.
...
- Landblock product commits/evictions still reach the renderer, but through the stable product source/event subscription.
- Product event tests pass both before and after moving coordinator ownership.

Progress:

- Added `createSceneResourceRuntime(...)` as a neutral composition helper for `ClientAssetRuntime` plus `LandblockProductRuntime`.
- Moved `StaticLandblockRenderArtifactCoordinator` construction and disposal out of `BrowserWorldDisplay.svelte` and into `App.svelte` scene-resource runtime construction.
- `App.svelte` now syncs assets and landblock products together from neutral browser scene interest, and passes only `staticLandblockProductSource` into `BrowserWorldDisplay`.
- Removed product-runtime sync and disposal from `BrowserWorldDisplay.svelte`; its scene-resource update path now updates renderer presentation/resources only.

Verification:

- `npm run --prefix apps/holtburger-3d test:ts -- src/lib/scene-runtime/scene-resource-runtime.test.ts src/lib/world-display/static-landblock-render-artifact-coordinator.test.ts src/lib/world-display/static-landblock-product-source.test.ts` passed: 3 files, 12 tests.
- `npm run --prefix apps/holtburger-3d test:ts` passed: 99 files, 528 tests.
- `npm run --prefix apps/holtburger-3d lint:ts` passed.
- `npm run --prefix apps/holtburger-3d check` passed with 0 errors and 0 warnings.
- `rg "new StaticLandblockRenderArtifactCoordinator|staticLandblockRenderCoordinator|syncSceneInterest\\(" apps/holtburger-3d/src/pages/BrowserWorldDisplay.svelte apps/holtburger-3d/src/App.svelte apps/holtburger-3d/src/lib/scene-runtime` shows coordinator construction/sync in `App.svelte` / neutral runtime only, with no `BrowserWorldDisplay.svelte` matches.
...
- Delete temporary product forwarding bridges, dual paths, or runtime composition shims introduced earlier in Phase 22. Any delayed deletion must be registered in Phase 23 before Phase 22 is marked complete.
...
- Phase 22 may briefly carry product-source event subscriptions alongside existing browser-display forwarding while `WorldDisplay` and runtime ownership cut over. Any such dual path must be short-lived and explicitly tracked.
...
| --- | --- | --- | --- | --- | --- |
| removed | Phase 21C/21G / Phase 23C | Transitional resolver snapshot/read-model adapters and record-snapshot test fixtures survived the renderer cutover. Phase 23C deleted the legacy prepared-cache snapshot helpers, removed renderer record helper APIs, moved test resolver helpers out of production `src`, and migrated off-frame planners/diagnostics to `PreparedAssetResolver`. | `prepared-asset-store.ts`, `renderer-asset-read-model.ts`, `scene-asset-streaming-controller.ts`, `material-diagnostics.ts`, `scene-asset-request-planner.ts`, `static-bundle-layer-planner.ts`, `apps/holtburger-3d/test-support/prepared-asset-resolver.ts` | No production source exports or consumes legacy prepared-cache snapshots; tests use resolver-backed helpers outside `src`. | `rg "createPreparedAssetLegacySnapshotFromResolver|createPreparedAssetResolverFromRecordSnapshot|createRendererAssetReadModelFromRecords|createLegacySnapshot|PreparedAssetLegacySnapshot|preparedByAssetId: \\{\\}|cacheMetadataByAssetId: \\{\\}" apps/holtburger-3d/src apps/holtburger-3d/test-support` returns no legacy helper or empty fixture matches. |
| removed | Phase 22B / Phase 23A | Asset coverage request planning previously consumed browser-shaped destinations behind `createBrowserDestinationFromSceneResourceInterest(...)` while `SceneAssetStreamingController` accepted neutral `SceneResourceInterest`. Phase 23A migrated coverage request planning, coverage retention ids, and material mismatch diagnostics to neutral `SceneResourceInterest` inputs; browser-mode wrappers now live only at the app/browser adapter boundary. | `scene-asset-streaming-controller.ts`, `scene-asset-request-planner.ts`, `browser-scene-resource-interest.ts` | `createSceneCoverageRequests()`, `deriveSceneCoverageAssetIds()`, and material visibility helpers accept `SceneResourceInterest` or a neutral planner input directly. | `rg "createBrowserDestinationFromSceneResourceInterest" apps/holtburger-3d/src/lib/assets apps/holtburger-3d/src/lib/scene-runtime apps/holtburger-3d/src/app` returned no matches. |
...
| removed | Phase 21D / Phase 23B | Lower-level WebGL resource builders previously consumed a renderer-owned legacy `AssetChannelState` snapshot because terrain/material/readiness APIs had not all moved to resolver inputs. Phase 23B migrated renderer resource/readiness paths to `RendererAssetReadModel` / `PreparedAssetResolver` reads and deleted the renderer snapshot helpers. | `webgl2-world-display-renderer-impl.ts`, `webgl2-world-resources.ts`, `world-render-frame.ts`, `terrain-blend-plan.ts`, `render-material-strategy.ts`, `static-renderables.ts`, `structured-interior-scene.ts`, `transition-portal-work-items.ts`, `terrain-render-artifact.ts` | Lower-level resource/readiness builders accept resolver/read-model inputs directly; renderer no longer snapshots the prepared cache into `AssetChannelState`. | `rg "createAssetChannelStateSnapshotFromResolver|createLegacyAssetStateSnapshot" apps/holtburger-3d/src` returned no matches. `rg "AssetChannelState" apps/holtburger-3d/src/lib/world-display --glob '!*.test.ts'` only shows presentation/report paths in `model.ts` and `browser-render-resource-coordinator.ts`. |
...
| removed | Phase 22A/22C | Temporary dual path where product-source events and browser-display product forwarding both existed during `WorldDisplay` constructor-source cutover. Phase 22C made `WorldDisplay` subscribe to `StaticLandblockProductSource` and deleted browser-display product forwarding. | `static-landblock-product-source.ts`, `static-landblock-render-artifact-coordinator.ts`, `BrowserWorldDisplay.svelte`, `WorldDisplay.svelte`, `world-display-renderer.ts` | Product-source events are the only path from landblock product runtime to `WorldDisplay` / renderer. | `rg "worldDisplaySurface\\?\\.(commitStaticLandblockProduct|evictStaticLandblockProduct|clearStaticLandblockProducts)" apps/holtburger-3d/src` has no matches. |
| removed | Phase 22D | Runtime ownership bridge while `StaticLandblockRenderArtifactCoordinator` was moved out of `BrowserWorldDisplay.svelte`. `App.svelte` now composes asset and landblock product runtimes through `SceneResourceRuntime`; `BrowserWorldDisplay.svelte` receives only product-source read/event access. | `App.svelte`, `scene-resource-runtime.ts`, `BrowserWorldDisplay.svelte` | Neutral `LandblockProductRuntime` / `SceneResourceRuntime` owns coordinator/store/worker lifecycle. | `rg "new StaticLandblockRenderArtifactCoordinator" apps/holtburger-3d/src/pages/BrowserWorldDisplay.svelte` has no matches. |
| removed | Phase 22B/22D | Temporary shared scene-resource update function could trigger product sync from presentation-only changes. Phase 22D moved product runtime sync to `App.svelte`'s scene-interest subscription, while `BrowserWorldDisplay.svelte` updates only renderer presentation/resource inputs. | `BrowserWorldDisplay.svelte`, `App.svelte` | Scene-interest sync and presentation-only updates are separate call paths, with runtime sync consuming a neutral scene-interest DTO. | `rg "syncSceneInterest\\(" apps/holtburger-3d/src/pages/BrowserWorldDisplay.svelte` has no matches. |

### Phase 23A: Remove Browser-Destination Planner Bridge From Runtime

Status: complete.

Deliverables:

- Add neutral scene asset planner inputs based on `SceneResourceInterest` / `SceneResourceLocation`.
- Keep browser-mode adapters at the app boundary only, so existing browser UI tests and callers can still derive neutral planner input without the runtime round-tripping through `BrowserLocationSelection`.
- Update `SceneAssetStreamingController` to call neutral coverage/material planning directly.
- Delete `createBrowserDestinationFromSceneResourceInterest(...)` and its test coverage once no runtime planner bridge uses it.
- Update the cleanup ledger row for the Phase 22B bridge when the grep proof is clean.

Acceptance criteria:

- `rg "createBrowserDestinationFromSceneResourceInterest" apps/holtburger-3d/src/lib/assets apps/holtburger-3d/src/lib/scene-runtime apps/holtburger-3d/src/app` returns no matches.
- Focused scene asset planner and streaming-controller tests pass.
- `npm run --prefix apps/holtburger-3d lint:ts` and `npm run --prefix apps/holtburger-3d check` pass.

Implementation notes:

- `scene-asset-request-planner.ts` now exposes neutral `createSceneCoverageRequests()`, `deriveSceneCoverageAssetIds()`, `deriveVisibleMaterialAssetIdsForSceneInterest()`, and `deriveAllVisibleMaterialAssetIdsForSceneInterest()` entrypoints keyed by `SceneResourceInterest`.
- Browser-shaped request/coverage/material wrappers remain explicit as `createBrowserSceneCoverageRequests()` and `deriveBrowserSceneCoverageAssetIds()` for browser-mode callers/tests, but the runtime no longer round-trips neutral scene interest back into `BrowserLocationSelection`.
- `SceneAssetStreamingController` now passes `SceneResourceInterest` directly through coverage request planning, active coverage retention id derivation, and material mismatch diagnostics.
- Deleted `createBrowserDestinationFromSceneResourceInterest(...)`; the browser adapter is one-way from browser state into neutral scene interest.

Verification notes:

- Focused tests passed:
  - `npm run --prefix apps/holtburger-3d test:ts -- src/lib/assets/scene-asset-request-planner.test.ts src/lib/assets/scene-asset-streaming-controller.test.ts src/app/browser-scene-resource-interest.test.ts`
- `npm run --prefix apps/holtburger-3d lint:ts` passed.
- `npm run --prefix apps/holtburger-3d check` passed with zero errors/warnings.
- Bridge grep returned no matches:
  - `rg "createBrowserDestinationFromSceneResourceInterest" apps/holtburger-3d/src/lib/assets apps/holtburger-3d/src/lib/scene-runtime apps/holtburger-3d/src/app`

### Phase 23B: Remove Renderer AssetChannelState Compatibility Snapshot

Status: complete.

Goal:

- Finish the Phase 21D cutover by removing the renderer-owned `AssetChannelState` compatibility snapshot from lower-level WebGL resource/readiness paths.

Context:

- Phase 21D removed the public renderer `setAssetState(...)` contract and stopped pushing Svelte-owned full cache state into renderer hot paths.
- Before this phase, the WebGL renderer still created a local `AssetChannelState` snapshot from `PreparedAssetResolver` when prepared-record events flushed, then passed that legacy shape through lower-level builders.
- That was better than Svelte owning the cache, but still structurally wrong: resource/readiness helpers should read prepared assets through resolver/read-model inputs, not record-shaped `AssetChannelState`.
- The original grep showed active source debt in `webgl2-world-display-renderer-impl.ts`, `webgl2-world-resources.ts`, `world-render-frame.ts`, `terrain-blend-plan.ts`, `render-material-strategy.ts`, `static-renderables.ts`, `structured-interior-scene.ts`, `transition-portal-work-items.ts`, `terrain-render-artifact.ts`, plus diagnostic/presentation compatibility in `material-diagnostics.ts`.

Deliverables:

1. Introduce a small renderer asset read model, likely based on `PreparedAssetResolver`, for lower-level world-display helpers.
   - It must provide direct `get(assetId)`/`has(assetId)` semantics and bounded iteration only where an existing helper genuinely scans prepared records.
   - It must not expose Svelte state, mutable `preparedByAssetId`, or `cacheMetadataByAssetId`.
2. Migrate terrain helpers first.
   - `terrain-render-artifact.ts`
   - `terrain-blend-plan.ts`
   - terrain-related sections of `webgl2-world-resources.ts`
   - terrain readiness tests
3. Migrate material/static helpers next.
   - `render-material-strategy.ts`
   - `render-material-plans.ts`
   - `material-texture-resolution.ts`
   - `region-detail-overlays.ts`
   - `static-renderable-readiness.ts`
   - `static-renderables.ts`
4. Migrate structured interior and portal helpers.
   - `structured-interior-scene.ts`
   - `transition-portal-work-items.ts`
   - `scene-renderable-readiness.ts`
   - `world-render-frame.ts`
   - `render-bvh-visibility-snapshot.ts`
5. Remove renderer snapshot creation.
   - Delete `createAssetChannelStateSnapshotFromResolver(...)` usage from `webgl2-world-display-renderer-impl.ts`.
   - Delete `assetState` compatibility plumbing in `webgl2-world-resources.ts` once all helper inputs use the read model.
6. Triage remaining `AssetChannelState` source references.
	- Keep compact presentation/report inputs only where they are genuinely diagnostic or UI state.
	- Keep `material-diagnostics.ts` as explicit off-frame diagnostic compatibility if it still consumes full prepared records for material visibility, and keep it tracked in the Phase 23 cleanup ledger.
	- Delete compatibility tests that build `AssetChannelState` solely to exercise renderer resource helpers; replace them with resolver/read-model fixtures.
7. Update the cleanup ledger row from `active` to `removed` only after the source grep proves runtime renderer paths no longer depend on `AssetChannelState`.

Acceptance criteria:

- `webgl2-world-display-renderer-impl.ts` no longer imports or calls `createAssetChannelStateSnapshotFromResolver(...)`.
- `rg "createAssetChannelStateSnapshotFromResolver" apps/holtburger-3d/src/lib/world-display apps/holtburger-3d/src/lib/assets` returns no renderer/runtime matches.
- `rg "AssetChannelState" apps/holtburger-3d/src/lib/world-display` returns no production renderer/resource helper matches, except explicitly justified presentation/report paths if any remain.
- Renderer resource lookup depends on `PreparedAssetResolver` or the new read model, not record-shaped full cache snapshots.
- Existing scene reports preserve material/terrain/static/interior readiness behavior.

Verification:

- Focused tests for migrated helper clusters:
  - terrain artifact/blend/resource tests
  - material/static readiness and render strategy tests
  - structured interior, transition portal, render-frame, and BVH visibility tests
- Full verification:
  - `npm run --prefix apps/holtburger-3d test:ts`
  - `npm run --prefix apps/holtburger-3d lint:ts`
  - `npm run --prefix apps/holtburger-3d check`
...
- Added `RendererAssetReadModel` as a renderer-local alias over `PreparedAssetResolver`; it is a read contract, not a cache owner.
- `webgl2-world-display-renderer-impl.ts` now keeps a resolver-backed read model and flushes prepared asset changes without constructing a full `AssetChannelState` compatibility snapshot.
- Terrain, material, static, structured-interior, portal, BVH, and world-frame helper chains now accept resolver/read-model inputs directly.
- Deleted `createAssetChannelStateSnapshotFromResolver(...)` and `createLegacyAssetStateSnapshot(...)`; remaining `AssetChannelState` references under `world-display` are presentation/report state only.
- Phase 23C later deleted the worker/test record helper and the prepared-cache legacy snapshot helpers rather than keeping them as diagnostic compatibility.

Verification notes:

- Focused migrated helper cluster passed:
  - `npm run --prefix apps/holtburger-3d test:ts -- src/lib/world-display/terrain-render-artifact.test.ts src/lib/world-display/terrain-blend-plan.test.ts src/lib/world-display/webgl2-world-resources.test.ts src/lib/world-display/render-material-strategy.test.ts src/lib/world-display/indexed-material-data.test.ts src/lib/world-display/static-renderables.test.ts src/lib/world-display/static-renderable-readiness.test.ts src/lib/world-display/structured-interior-scene.test.ts src/lib/world-display/transition-portal-work-items.test.ts src/lib/world-display/scene-renderable-readiness.test.ts src/lib/world-display/world-render-frame.test.ts src/lib/world-display/render-bvh-visibility-snapshot.test.ts`
  - Result: 12 files passed, 77 tests passed.
- Full validation passed:
  - `npm run --prefix apps/holtburger-3d test:ts` returned 100 files passed, 530 tests passed.
  - `npm run --prefix apps/holtburger-3d lint:ts` passed.
  - `npm run --prefix apps/holtburger-3d check` passed with zero Svelte/TS errors or warnings.
- Grep proof:
  - `rg "createAssetChannelStateSnapshotFromResolver|createLegacyAssetStateSnapshot" apps/holtburger-3d/src` returned no matches.
  - `rg "assetState|preparedByAssetId|cacheMetadataByAssetId" apps/holtburger-3d/src/lib/world-display/webgl2-world-display-renderer-impl.ts apps/holtburger-3d/src/lib/world-display/webgl2-world-resources.ts` returned no matches.
  - `rg "AssetChannelState" apps/holtburger-3d/src/lib/world-display --glob '!*.test.ts'` only shows presentation/report paths in `model.ts` and `browser-render-resource-coordinator.ts`.
  - Phase 23C later removed the remaining prepared-cache legacy snapshot helper and migrated material diagnostics to resolver reads.
- Final grep proof recorded in this worksheet and the cleanup ledger.

### Phase 23C: Delete Legacy Prepared-Cache Fixtures And Production Helpers

Status: complete.

Goal:

- Remove the remaining record-snapshot prepared-cache helpers and tests so production code exposes only resolver/store-owned prepared asset access.

Deliverables:

- Delete legacy snapshot helpers from `prepared-asset-store.ts`.
- Delete renderer record-helper construction from `renderer-asset-read-model.ts`.
- Move the test prepared-asset resolver helper outside production `src`.
- Replace tests that used empty `preparedByAssetId` / `cacheMetadataByAssetId` fixture maps with resolver-backed setup.
- Migrate off-frame request planning, material diagnostics, and static bundle layer planning to `PreparedAssetResolver` inputs.
- Remove obsolete material signature functions/tests that only proved the old snapshot model.

Verification:

- `npm run --prefix apps/holtburger-3d check` passed with zero Svelte/TS errors or warnings.
- `npm run --prefix apps/holtburger-3d test:ts` passed: 99 files, 525 tests.
- `npm run --prefix apps/holtburger-3d lint:ts` passed.
- `rg "createPreparedAssetLegacySnapshotFromResolver|createPreparedAssetResolverFromRecordSnapshot|createRendererAssetReadModelFromRecords|createLegacySnapshot|PreparedAssetLegacySnapshot|preparedByAssetId: \\{\\}|cacheMetadataByAssetId: \\{\\}" apps/holtburger-3d/src apps/holtburger-3d/test-support` returned no matches.
- `find apps/holtburger-3d/src -type d -name test-support -print` returned no matches.

Course-correction guardrails:

- Do not replace `AssetChannelState` with a differently named object that still snapshots every prepared asset for renderer hot paths. That would be fake cleanup.
- Do not make the read model optional in renderer helper inputs for backwards compatibility. Update call sites and tests.
- If a helper truly needs a full immutable snapshot for off-frame diagnostics, name that API as a diagnostic/planning snapshot and keep it out of render-frame/resource rebuild paths.
