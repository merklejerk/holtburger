...
| Physical omega is never represented as a global rotation | Neither solver carries a quaternion angular velocity — one holds a heading scalar, the other takes an absolute heading — so the local-versus-global distinction has only one side implemented. Revisit when a solver needs to tumble a body about a non-vertical axis |
...
| The Bats root-rotation `RETAIL DIVERGENCE`: a `SetupModel::default_animation` resident whose clip carries authored root frames renders without them | **Diagnosed 2026-08-20, and it is a scope gap rather than an architectural one.** The contract covers `setup_default_tables` — which motion *table* a setup installs — but not `SetupModel::default_animation`, a bare animation a setup plays when it has no table at all. Confirmed against the catalog: WCID 36449 declares no motion table, so nothing routes its root frames anywhere. The fix is one more projection source into the same contract, giving setup-default clips a `RootMotionTrack` the existing authored-drive path already knows how to apply — not a new mechanism. Deliberately not taken here: the population is one canonical template with rotation-only, zero-translation frames, and the projection would touch every setup-default resident to serve it. Revisit if a second carrier appears, or fold into whatever next needs setup-default motion |
...
- [~] The authored contribution is one exactly-composed rigid offset, with authored rotation applied
      locally and physical omega globally. *Local composition is proven; no solver represents
      physical omega as a global rotation, recorded as debt.*
...
- [~] The Explorer capability boundary and the Bats root-rotation divergence are removed. *The
      capability boundary is gone. The Bats divergence is **not** removed and was misfiled here: it
      is a `SetupModel::default_animation` resident whose clip carries root frames, and static
      scenery has no host body to route them through. Motion-table entities never had the problem.
      Carried to the debt list rather than closed.*
