...
| :--- | :--- | :--- | :--- |
| 0 | uint32 | Sequence | Rolling sequence number for the packet. |
...
## 4. Message Fragments
When `BlobFragments` is set, the data portion (after optional headers) consists of one or more fragments.

### 4.1 Fragment Header (16 bytes)

| Offset | Type | Name | Description |
| :--- | :--- | :--- | :--- |
| 0 | uint32 | Sequence | Unique index for this message segment. |
| 4 | uint32 | Id | Unique ID for the message being sent. |
| 8 | uint16 | Count | Total fragments needed for this message. |
| 10 | uint16 | Size | Size of payload in THIS fragment (including this header). |
| 12 | uint16 | Index | 0-based index of this fragment. |
| 14 | uint16 | Queue | Destination queue (e.g. `0x01` Game, `0x02` UI). |

## 5. Checksum and ISAAC

### 5.1 Hash32 Algorithm
```csharp
...
### 5.2 Packet Checksum (Unencrypted)
...
3. `packet.Checksum = header_hash + payload_hash`.

### 5.3 Packet Checksum (Encrypted)
If `EncryptedChecksum` **is** set:

1.  **Header Hash:** `header_hash = Hash32(header)` (computed with the `checksum` field set to the seed `0xBADD70DD`).
...
3.  **ISAAC Mask:** `key = ISAAC.Next()` (consumes one word from the stream).
4.  **Final Checksum:** `packet.Checksum = header_hash.wrapping_add(payload_hash ^ key)`.

**Note on Composite Hashing:**
ACE does NOT hash the raw payload bytes for encrypted packets. It hashes each component separately and sums the results. This handles the 4-byte alignment of fragments naturally, as padding bytes between fragments are NOT included in any component's `Hash32`.
