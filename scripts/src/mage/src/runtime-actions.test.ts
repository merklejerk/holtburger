...
test("combat engagement only tracks living targets", () => {
  ...
  assert.equal(
    isCombatTargetAlive(
      { motionCommand: { kind: "dead" } } as unknown as ScriptEntityView,
    ),
    false,
  );
  assert.equal(
    isCombatTargetAlive(
      { motionCommand: { kind: "walking" } } as unknown as ScriptEntityView,
    ),
    true,
  );
});
...
test("vulnerability landing chat messages resolve the cast and update cooldown history", () => {
  ...
  const handled = resolveSpellcastFromChatMessage(
    ...
    {
      sender: "You",
      message: "You cast Fire Vulnerability on Goblin, surpassing Acid Vulnerability.",
    },
    "Goblin",
  );
...
function sampleMonsterCandidate(targetGuid: Guid, x: number, y: number): MonsterCandidate {
  return {
    guid: targetGuid,
    weenieId: 1,
