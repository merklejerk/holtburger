...
export function interruptSpellcast(
  ...
  return finalizeSpellcast(state, {
    kind: "interrupted",
    reason,
  }) != null;
}
...
export function failedVulnAttemptCount(
  ...
  return state.vulnerabilityPolicy.failedVulnAttemptsByTarget.get(targetGuid) ?? 0;
}
...
function incrementVulnerabilityAttempt(
  ...
  state.vulnerabilityPolicy.failedVulnAttemptsByTarget.set(targetGuid, nextAttemptCount);
